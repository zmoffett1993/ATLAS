...
test('activation removes obsolete ATLAS cache versions and preserves unrelated caches',async()=>{
 ...
 assert.equal(f.stores.has('atlas-pwa-v353-account-transaction-warehouse-data'),false);assert.equal(f.stores.has('unrelated-cache'),true);assert.ok([...f.stores.keys()].some(k=>k.includes('v356')));
});
