(function(){
  ...
  function officialSheet(record){const snap=snapshot(record),rows=(snap.pallets||[]).map((pallet)=>{const order=[],groups=new Map();(pallet.lots||[]).forEach((lot)=>{const model=String(lot.model||"UNKNOWN SKU");if(!groups.has(model)){groups.set(model,[]);order.push(model)}groups.get(model).push(lot)});return order.map((model)=>{const lots=groups.get(model),total=lots.reduce((sum,lot)=>sum+Number(lot.cases||0)*Number(lot.caseQuantity||0),0);return `<tr class="receiver-sheet-model"><th>${esc(model)}</th><th>PALLET ${pallet.number}</th><td></td></tr>${lots.map((lot)=>`<tr><td></td><td>${esc(lot.lot||"—")}</td><td>${(Number(lot.cases||0)*Number(lot.caseQuantity||0)).toLocaleString()}</td></tr>`).join("")}<tr class="receiver-sheet-total"><td></td><th>TOTAL QTY</th><td>${total.toLocaleString()}</td></tr>`}).join("")}).join("");return `<section class="receiver-official-sheet"><h2>Certificate of Compliance Information Form</h2><div class="receiver-sheet-fields"><div><strong>CUSTOMER NAME</strong><span>${esc(snap.customerName||"—")}</span></div><div><strong>INV-NUMBER</strong><span>${esc(snap.invoiceNumber||"—")}</span></div><div><strong>IF-NUMBER</strong><span>${esc(snap.ifNumber||"—")}</span></div></div><div class="receiver-sheet-table"><table><thead><tr><th>MODEL NUMBER</th><th>LOT NUMBER<br><small>(Pallet listed for each model)</small></th><th>QUANTITY</th></tr></thead><tbody>${rows}</tbody></table></div></section>`}
  function officialPreviewMarkup(){const snap=snapshot(selected);return `<div class="receiver-shell">${header()}<button class="receiver-back" data-action="back-detail">‹ COC Overview</button><section class="receiver-preview-head"><span class="receiver-eyebrow">READ-ONLY PREVIEW</span><h1>Official COC</h1><p>${esc(snap.customerName||"—")} · ${esc(snap.invoiceNumber||"—")}</p></section>${officialSheet(selected)}<div class="receiver-preview-download"><button class="receiver-primary receiver-download" data-action="download" data-id="${esc(selected.id)}">DOWNLOAD OFFICIAL COC</button></div></div>`}
  function detailMarkup(){const record=selected,snap=snapshot(record),archived=Boolean(record.receiver_archived_at);return `<div class="receiver-shell">${header()}<button class="receiver-back" data-action="back">‹ ${screen==="archive"?"COC Archive":"COC Receiver"}</button><section class="receiver-detail"><div class="receiver-detail-head"><span class="receiver-eyebrow ${archived?"is-archived":""}">${archived?"ARCHIVED":record.status==="OFFICE_COMPLETED"?"COMPLETED":"RECEIVED"}</span><h1>${esc(snap.customerName||"—")}</h1><p>${esc(snap.invoiceNumber||"—")}</p></div><dl><div><dt>Invoice</dt><dd>${esc(snap.invoiceNumber||"—")}</dd></div><div><dt>IF Number</dt><dd>${esc(snap.ifNumber||"—")}</dd></div><div><dt>Sent by</dt><dd>${esc(snap.employee||"—")} · ${time(record.sent_at)}</dd></div><div><dt>Received</dt><dd>${dateTime(record.received_at)}</dd></div></dl>${palletMarkup(record)}<div class="receiver-actions"><div class="receiver-official-actions"><button class="receiver-view-official" data-action="view-official" data-id="${esc(record.id)}">VIEW OFFICIAL COC</button><button class="receiver-primary receiver-download" data-action="download" data-id="${esc(record.id)}">DOWNLOAD OFFICIAL COC</button></div>${record.status!=="OFFICE_COMPLETED"?`<button class="receiver-primary receiver-complete" data-action="complete" data-id="${esc(record.id)}">MARK COMPLETED ✓</button>`:archived?`<button class="receiver-primary receiver-restore" data-action="restore-one" data-id="${esc(record.id)}">RESTORE TO COMPLETED</button>`:`<button class="receiver-primary receiver-archive-action" data-action="archive-one" data-id="${esc(record.id)}">ARCHIVE COC</button>`}</div></section></div>`}
  ...
  function render(){root.innerHTML=(!credentials?pairingMarkup():selected&&preview?officialPreviewMarkup():selected?detailMarkup():screen==="archive"?archiveMarkup():inboxMarkup())+retentionMarkup()+dialogMarkup()+noticeMarkup()}
  ...
  function setScreen(next){screen=next;selected=null;preview=false;selectedIds.clear();openMenu=null;bulkMenu=false;search="";page=1;render();loadInbox()}
  ...
  root.addEventListener("click",async(event)=>{
    ...
    if(action==="open"){selected=[...activeDeliveries,...completedDeliveries].find((item)=>item.id===id)||null;preview=false;openMenu=null;render()}
    if(action==="back"){selected=null;preview=false;render()}
    if(action==="view-official"){preview=true;render()}
