(function(){
  ...
  function render(){root.innerHTML=(!credentials?pairingMarkup():completionState?completionMarkup():selected?officialPreviewMarkup():screen==="archive"?archiveMarkup():inboxMarkup())+(completionState?"":retentionMarkup()+dialogMarkup()+noticeMarkup());window.requestAnimationFrame?.(()=>window.AtlasCocExcel?.fitOfficialWorkbookPreviews?.(root))}
  ...
  function finishReceiverWorkflow(){const snap=snapshot(selected);screen="inbox";completionState={customerName:snap.customerName||"",invoiceNumber:snap.invoiceNumber||""};selected=null;preview=false;previewState={status:"idle",html:"",error:"",id:""};revisionState=freshRevision();selectedIds.clear();openMenu=null;bulkMenu=false;dialog=null;notice=null;search="";period="today";page=1;reportingDayKey=reportingDateKey();render();positionWorkflowCompletion();void loadInbox();setTimeout(()=>{completionState=null;render();window.scrollTo({top:0,left:0,behavior:"auto"})},4000)}
