(function(){
  ...
  const SORT_OPTIONS=[
    ...
    ["invoice-asc","Invoice A–Z"],["invoice-desc","Invoice Z–A"],
    ["if-asc","IF number A–Z"],["if-desc","IF number Z–A"],
  ];
  let credentials=null,activeDeliveries=[],completedDeliveries=[],selected=null;
  let connection="reconnecting",pairing=null,pollTimer=null,subscription=null,lastSynced=null;
  let screen="inbox",search="",sort=readPreference("sort")||"newest",page=1,total=0;
  ...
  function header(){return `<header class="receiver-head"><div class="receiver-brand"><div class="receiver-atlas-lockup" aria-label="ATLAS Warehouse Management"><img src="../atlas-brand-mark-dark.svg?v=97" alt="" aria-hidden="true"><span><b>ATLAS</b><small>WAREHOUSE MANAGEMENT</small></span></div><div class="receiver-brand-title"><h1>COC RECEIVER</h1><p>Office COC Station</p></div></div><div class="receiver-status ${connection==="connected"?"":"is-offline"}"><strong>● ${connection==="connected"?"CONNECTED · READY":connection==="reconnecting"?"RECONNECTING…":"OFFLINE"}</strong><small>Last synced ${lastSynced?time(lastSynced):"—"}</small></div></header>`}
  ...
  function incomingCard(record){const snap=snapshot(record),totals=recordTotals(record);return `<article class="receiver-incoming-card"><span class="receiver-new">NEW</span><dl><div><dt>Invoice</dt><dd>${esc(snap.invoiceNumber||"—")}</dd></div><div><dt>Customer</dt><dd>${esc(snap.customerName||"—")}</dd></div><div><dt>IF number</dt><dd>${esc(snap.ifNumber||"—")}</dd></div></dl><p>${icon("clipboard")} ${plural(totals.pallets,"pallet")} <b>·</b> ${plural(totals.boxes,"box")}</p><p>${esc(snap.employee||"—")} <b>·</b> ${time(record.sent_at)}</p><button type="button" class="receiver-open-report" data-action="open" data-id="${esc(record.id)}">OPEN REPORT <span>→</span></button></article>`}
  ...
  function toolbarMarkup({archive=false}={}){const count=selectedIds.size;return `<div class="receiver-toolbar"><label class="receiver-search">${icon("search")}<input type="search" data-receiver-search value="${esc(search)}" placeholder="Search customer, invoice, or IF number" autocomplete="off"></label><label class="receiver-sort"><span class="sr-only">Sort reports</span><select data-receiver-sort>${sortOptions()}</select></label><div class="receiver-toolbar-actions"><button type="button" class="receiver-outline" data-action="${archive?"show-inbox":"show-archive"}">${archive?"BACK TO RECEIVER":"VIEW ARCHIVE"}</button><button type="button" class="receiver-outline ${archive?"is-restore":""}" data-action="${archive?"restore-selected":"archive-selected"}" ${count?"":"disabled"}>${archive?"RESTORE":"ARCHIVE"} SELECTED${count?` (${count})`:""}</button>${archive?"":`<div class="receiver-bulk-menu"><button type="button" class="receiver-icon-button" data-action="toggle-bulk-menu" aria-label="More archive actions" aria-expanded="${bulkMenu}">•••</button>${bulkMenu?`<div class="receiver-menu"><button type="button" data-action="archive-all">Archive all completed COCs</button></div>`:""}</div>`}</div></div>`}
  ...
  function reportsPanel({archive=false}={}){const from=total?((page-1)*PAGE_SIZE)+1:0,to=Math.min(page*PAGE_SIZE,total),allChecked=completedDeliveries.length&&completedDeliveries.every((item)=>selectedIds.has(item.id));return `<section class="receiver-report-panel"><div class="receiver-panel-title"><div><span class="receiver-eyebrow">${archive?"SECURE RECORD STORAGE":"OFFICE HISTORY"}</span><h2>${archive?"COC ARCHIVE":"COMPLETED COCs"}</h2><p>${archive?"Archived reports remain securely stored, downloadable, and restorable.":"Completed reports remain available until archived."}</p></div></div>${toolbarMarkup({archive})}<div class="receiver-table-wrap"><table><thead><tr><th><input type="checkbox" data-select-page aria-label="Select all reports on this page" ${allChecked?"checked":""}></th><th>DATE COMPLETED ↓</th><th>CUSTOMER</th><th>INVOICE</th><th>IF NUMBER</th><th>PALLETS / BOXES</th><th>ACTIONS</th></tr></thead><tbody>${completedDeliveries.length?completedDeliveries.map((item)=>rowMarkup(item,{archive})).join(""):`<tr><td colspan="7"><div class="receiver-table-empty">${search?"No reports match this search.":archive?"No COCs have been archived.":"No completed COCs yet."}</div></td></tr>`}</tbody></table></div><footer class="receiver-pagination"><span>Showing ${from}–${to} of ${total.toLocaleString()} ${archive?"archived":"completed"} COCs</span><nav aria-label="Report pages">${pageButtons()}</nav></footer></section>`}
  function inboxMarkup(){return `<div class="receiver-shell">${header()}${metricsMarkup()}${incomingMarkup()}${reportsPanel()}</div>`}
  function archiveMarkup(){return `<div class="receiver-shell">${header()}<div class="receiver-archive-head"><button type="button" data-action="show-inbox">‹ Back to Receiver</button><div><span class="receiver-eyebrow">RETAINED RECORDS</span><h1>COC Archive</h1><p>Search, download, or restore any archived compliance report.</p></div></div>${reportsPanel({archive:true})}</div>`}
  function palletMarkup(record){return (snapshot(record).pallets||[]).map((pallet)=>`<section class="receiver-pallet"><h3>PALLET ${pallet.number}</h3>${(pallet.lots||[]).map((lot)=>`<div class="receiver-lot"><span><strong>${esc(lot.model)}</strong><small>Lot ${esc(lot.lot)}</small></span><b>${plural(Number(lot.cases||0),"box")} · ${(Number(lot.cases||0)*Number(lot.caseQuantity||0)).toLocaleString()} units</b></div>`).join("")}</section>`).join("")}
  function detailMarkup(){const record=selected,snap=snapshot(record),archived=Boolean(record.receiver_archived_at);return `<div class="receiver-shell">${header()}<button class="receiver-back" data-action="back">‹ ${screen==="archive"?"COC Archive":"COC Receiver"}</button><section class="receiver-detail"><div class="receiver-detail-head"><span class="receiver-eyebrow ${archived?"is-archived":""}">${archived?"ARCHIVED":record.status==="OFFICE_COMPLETED"?"COMPLETED":"RECEIVED"}</span><h1>${esc(snap.customerName||"—")}</h1><p>${esc(snap.invoiceNumber||"—")}</p></div><dl><div><dt>Invoice</dt><dd>${esc(snap.invoiceNumber||"—")}</dd></div><div><dt>IF Number</dt><dd>${esc(snap.ifNumber||"—")}</dd></div><div><dt>Sent by</dt><dd>${esc(snap.employee||"—")} · ${time(record.sent_at)}</dd></div><div><dt>Received</dt><dd>${dateTime(record.received_at)}</dd></div></dl>${palletMarkup(record)}<div class="receiver-actions"><button class="receiver-primary receiver-download" data-action="download" data-id="${esc(record.id)}">DOWNLOAD COMPANY COC</button>${record.status!=="OFFICE_COMPLETED"?`<button class="receiver-primary receiver-complete" data-action="complete" data-id="${esc(record.id)}">MARK COMPLETED ✓</button>`:archived?`<button class="receiver-primary receiver-restore" data-action="restore-one" data-id="${esc(record.id)}">RESTORE TO COMPLETED</button>`:`<button class="receiver-primary receiver-archive-action" data-action="archive-one" data-id="${esc(record.id)}">ARCHIVE COC</button>`}</div></section></div>`}
  ...
  function render(){root.innerHTML=(!credentials?pairingMarkup():selected?detailMarkup():screen==="archive"?archiveMarkup():inboxMarkup())+retentionMarkup()+dialogMarkup()+noticeMarkup()}
  ...
  function setScreen(next){screen=next;selected=null;selectedIds.clear();openMenu=null;bulkMenu=false;search="";page=1;render();loadInbox()}
  ...
  async function download(button){button.disabled=true;try{const workbook=await Delivery.downloadOfficeWorkbook(button.dataset.id,credentials);window.AtlasCocStorage?.downloadBlob(workbook.blob,workbook.fileName)}catch(error){showNotice(error?.message||"The official COC could not be downloaded.","error")}finally{button.disabled=false}}
  ...
  root.addEventListener("click",async(event)=>{
    ...
    if(action==="open"){selected=[...activeDeliveries,...completedDeliveries].find((item)=>item.id===id)||null;openMenu=null;render()}
    if(action==="back"){selected=null;render()}
