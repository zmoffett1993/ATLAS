(function(){
  ...
  function incomingCard(record){const snap=snapshot(record),totals=recordTotals(record);return `<article class="receiver-incoming-card"><span class="receiver-new">NEW</span><dl><div><dt>Customer Name</dt><dd>${esc(snap.customerName||"—")}</dd></div><div><dt>IF Number</dt><dd>${esc(snap.ifNumber||"—")}</dd></div><div><dt>INV Number</dt><dd>${esc(snap.invoiceNumber||"—")}</dd></div></dl><p>${icon("clipboard")} ${plural(totals.pallets,"pallet")} <b>·</b> ${plural(totals.boxes,"box")}</p><p>${esc(submitterName(record))} <b>·</b> ${time(record.sent_at)}</p><button type="button" class="receiver-open-report" data-action="open" data-id="${esc(record.id)}">OPEN REPORT <span>→</span></button></article>`}
