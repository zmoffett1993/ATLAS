(function(){
  ...
  let inboxLoaded=false,inboxFailed=false,pendingBackgroundRender=false;
  ...
  function accountDisplayName(){const user=Delivery.currentUser(),name=String(user?.user_metadata?.display_name||user?.app_metadata?.display_name||user?.app_metadata?.login_name||"").trim();return name&&!name.includes("@")&&name!==user?.id?name:"Office account"}
  function accountWarehouseName(){return (credentials?.warehouseCode||Delivery.requestedWarehouseCode())==="TX"?"Texas Warehouse":"California Warehouse"}
  function signOutMarkup(menu=false){return `<button type="button" class="receiver-account-sign-out" data-action="sign-out"${menu?' role="menuitem"':''}>${menu?'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 4H5v16h5M9 12h12m-4-4 4 4-4 4"/></svg>':''}Sign Out / Switch Account</button>`}
  function pairingAccountMarkup(){return `<div class="receiver-pair-account"><span>Signed in as <strong>${esc(accountDisplayName())}</strong></span><small>${esc(accountWarehouseName())}</small>${signOutMarkup()}</div>`}
  function accountMenuMarkup(){return `<button type="button" class="receiver-account-trigger" data-action="account-menu" aria-label="Receiver account menu" aria-haspopup="menu" aria-expanded="false" aria-controls="receiver-account-menu"><span><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5"/><path d="M5 21v-2a7 7 0 0 1 14 0v2"/></svg></span></button><div id="receiver-account-menu" class="receiver-account-menu" popover="manual" role="menu" aria-label="Receiver account"><div class="receiver-account-details" role="presentation"><small>RECEIVER ACCOUNT</small><strong>${esc(accountDisplayName())}</strong><span>${esc(accountWarehouseName())}</span></div>${signOutMarkup(true)}</div>`}
  function positionAccountMenu(){const menu=root.querySelector("#receiver-account-menu"),trigger=root.querySelector(".receiver-account-trigger");if(!menu?.matches(":popover-open")||!trigger)return;const rect=trigger.getBoundingClientRect();menu.style.left=`${Math.max(8,Math.min(rect.right-menu.offsetWidth,window.innerWidth-menu.offsetWidth-8))}px`;menu.style.top=`${Math.max(8,Math.min(rect.bottom+6,window.innerHeight-menu.offsetHeight-8))}px`}
  function closeAccountMenu({returnFocus=false}={}){const menu=root.querySelector("#receiver-account-menu"),trigger=root.querySelector(".receiver-account-trigger");if(!menu?.matches(":popover-open"))return;menu.hidePopover();trigger?.setAttribute("aria-expanded","false");if(returnFocus)trigger?.focus({preventScroll:true})}
  function toggleAccountMenu(){const menu=root.querySelector("#receiver-account-menu"),trigger=root.querySelector(".receiver-account-trigger");if(!menu)return;if(menu.matches(":popover-open")){closeAccountMenu();return}menu.showPopover();trigger.setAttribute("aria-expanded","true");positionAccountMenu();menu.querySelector('[role="menuitem"]')?.focus({preventScroll:true})}
  document.addEventListener("click",(event)=>{if(!event.target.closest?.(".receiver-account-trigger,.receiver-account-menu"))closeAccountMenu()});
  document.addEventListener("keydown",(event)=>{if(event.key==="Escape"&&root.querySelector("#receiver-account-menu")?.matches(":popover-open")){event.preventDefault();closeAccountMenu({returnFocus:true})}});
  window.addEventListener("resize",positionAccountMenu);
  window.addEventListener("scroll",positionAccountMenu,{passive:true});
  function header(){const ready=connection==="connected"&&inboxLoaded&&!inboxFailed;return `<header class="receiver-head"><div class="receiver-atlas-lockup" aria-label="ATLAS Warehouse Management"><img src="../atlas-brand-mark-dark.svg?v=97" alt="" aria-hidden="true"><span><b>ATLAS</b><small>WAREHOUSE MANAGEMENT</small></span></div><div class="receiver-brand-title"><h1>COC RECEIVER</h1><p>${esc(branchName())}</p></div><div class="receiver-status ${ready?"":"is-offline"}"><strong>● ${ready?"CONNECTED · READY":connection==="offline"?"OFFLINE":"RECONNECTING…"}</strong><small>Last synced ${lastSynced?time(lastSynced):"—"}</small></div>${accountMenuMarkup()}</header>`}
  function pairingMarkup(){
    if(!Delivery.getAuthSession())return `<section class="receiver-pair receiver-sign-in"><span class="receiver-eyebrow">OFFICE COC STATION</span><h1>Sign in to the COC Receiver</h1><p>Use your assigned ATLAS name and password on the office receiver computer.</p><form data-receiver-sign-in><label><span>ATLAS name</span><input name="login_name" autocomplete="username" autocapitalize="none" autocorrect="off" spellcheck="false" placeholder="Enter ATLAS name" required ${signInState.loading?"disabled":""}></label><label><span>Password</span><input name="password" type="password" autocomplete="current-password" placeholder="Enter password" required ${signInState.loading?"disabled":""}></label><p class="receiver-sign-in-error" role="alert">${esc(signInState.error)}</p><button class="receiver-primary" type="submit" ${signInState.loading?"disabled":""}>${signInState.loading?"SIGNING IN…":"SIGN IN"}</button></form></section>`;
    if(!pairing)return `<section class="receiver-pair"><span class="receiver-eyebrow">OFFICE COC STATION</span><h1>Pair this computer</h1><p>This browser needs supervisor approval before it can receive compliance reports.</p><button class="receiver-primary" data-action="start-pairing">Create Pairing Code</button>${pairingAccountMarkup()}</section>`;
    return `<section class="receiver-pair"><span class="receiver-eyebrow">PAIRING REQUEST</span><h1>Approve on a warehouse phone</h1><p>Workflows → Office COC Receiver</p><div class="receiver-code">${esc(pairing.pairingCode)}</div><div class="receiver-qr">${pairingQrMarkup()}</div><p>Expires ${time(pairing.expiresAt)}</p><p>${esc(pairing.status||"Waiting for supervisor approval…")}</p>${pairingAccountMarkup()}</section>`;
  }
  ...
  function incomingCard(record){const snap=snapshot(record),totals=recordTotals(record);return `<article class="receiver-incoming-card"><button type="button" class="receiver-open-report" data-action="open" data-id="${esc(record.id)}">OPEN REPORT <span>→</span></button><span class="receiver-new">PENDING</span><dl><div><dt>Customer Name</dt><dd>${esc(snap.customerName||"—")}</dd></div><div><dt>IF NUMBER</dt><dd>${esc(snap.ifNumber||"—")}</dd></div><div><dt>INV Number</dt><dd>${esc(snap.invoiceNumber||"—")}</dd></div><div><dt>Sales Order</dt><dd>${esc(snap.salesOrderNumber||"—")}</dd></div></dl><div class="receiver-incoming-meta"><p class="receiver-incoming-summary">${icon("clipboard")} ${plural(totals.pallets,"pallet")} <b>·</b> ${plural(totals.boxes,"box")}</p><p class="receiver-incoming-submitter">${esc(submitterName(record))} <b>·</b> ${time(record.sent_at)}</p></div></article>`}
  function incomingMarkup(){return `<section class="receiver-incoming"><h2>INCOMING COCs</h2>${activeDeliveries.length?activeDeliveries.map(incomingCard).join(""):`<div class="receiver-ready-strip"><span>${icon("check")}</span><div><strong>No COCs are waiting</strong><small data-receiver-ready-copy>${connection==="connected"&&inboxLoaded&&!inboxFailed?"The receiver is connected and ready for the next warehouse report.":"Connection not confirmed. Reports may not be current."}</small></div><button type="button" class="receiver-outline receiver-reset-pairing" data-action="reset-pairing" ${connection==="connected"&&inboxLoaded&&!inboxFailed?"hidden":""}>Set up this computer</button></div>`}</section>`}
  ...
  function positionWorkflowCompletion(){
    const place=()=>{if(!completionState)return;root.querySelector(".receiver-workflow-complete")?.scrollIntoView({block:"center",inline:"nearest",behavior:"auto"})};
    window.requestAnimationFrame?.(()=>window.requestAnimationFrame?.(place));
    window.setTimeout(place,140);
  }
  ...
  function updateReceiverStatus(){const ready=connection==="connected"&&inboxLoaded&&!inboxFailed,status=root.querySelector(".receiver-status");if(status){status.classList.toggle("is-offline",!ready);const strong=status.querySelector("strong"),small=status.querySelector("small");if(strong)strong.textContent=`● ${ready?"CONNECTED · READY":connection==="offline"?"OFFLINE":"RECONNECTING…"}`;if(small)small.textContent=`Last synced ${lastSynced?time(lastSynced):"—"}`}const copy=root.querySelector("[data-receiver-ready-copy]"),reset=root.querySelector("[data-action=reset-pairing]");if(copy)copy.textContent=ready?"The receiver is connected and ready for the next warehouse report.":"Connection not confirmed. Reports may not be current.";if(reset)reset.hidden=ready}
  ...
  async function loadInbox({foreground=false}={}){
    ...
    try{
      ...
      let [activeResult,listResult,metricResult]=await Promise.all([
        Delivery.receiverInbox(credentials,{section:"active",withMeta:true}),
      ...
      connection=navigator.onLine?"connected":"offline";lastSynced=new Date();const currentSignature=inboxSignature(),dataChanged=!inboxLoaded||periodReset||previousSignature!==currentSignature;inboxLoaded=true;inboxFailed=false;renderBackgroundUpdate(dataChanged,{foreground});
    }catch(error){if(sequence!==loadSequence)return;connection=navigator.onLine?"reconnecting":"offline";inboxFailed=true;notice={tone:"error",text:error?.message||"The COC Receiver could not refresh."};updateReceiverStatus()}
  ...
  async function loadRevisionStatus(id){try{const result=await Delivery.cocWorkbookRevision("status",{deliveryId:id},branchCode());if(!selected||selected.id!==id)return;revisionState.hasPendingRevision=Boolean(result.hasPendingRevision);revisionState.currentRevision=result.currentRevision||null;revisionState.revisions=Array.isArray(result.revisions)?result.revisions:[];revisionState.error=""}catch(error){if(selected?.id===id)revisionState.error=revisionError(error,"Workbook revision history is unavailable.")}}
  ...
  function validateNativeEditor(form,{allowPending=false}={}){
    const fields=[
    ...
    fields.forEach(([input,message,type,row])=>{
      if(!input)return;
      if(allowPending&&["invoiceNumber","ifNumber"].includes(input.name)){clearReceiverFieldError(input);return}
  ...
  function nativeWorkbookData({allowPending=false}={}){const editor=revisionState.editor;if(!editor?.customerName||(!allowPending&&(!editor.invoiceNumber||!editor.ifNumber)))throw new Error("COC_REVISION_HEADER_FIELDS_REQUIRED");if(!editor.lines?.length)throw new Error("COC_REVISION_DETAIL_ROWS_REQUIRED");const pallets=new Map();editor.lines.forEach((line)=>{const palletNumber=Math.max(1,Number(line.palletNumber)||1),sku=completeSkuRecord(line.model),model=sku?.modelNumber||"",lot=String(line.lot||"").trim().toUpperCase(),quantity=Number(line.quantity),boxCount=Math.max(0,Math.round(Number(line.boxCount)||0)),unitsPerBox=Math.max(0,Math.round(Number(line.unitsPerBox)||0));if(!sku)throw new Error("COC_NATIVE_EDITOR_SKU_INCOMPLETE");if(line.isNew&&(line.boxApplied!==true||boxCount<1||unitsPerBox<1||quantity!==boxCount*unitsPerBox))throw new Error("COC_NATIVE_EDITOR_BOX_COUNT_REQUIRED");if(!lot||!Number.isSafeInteger(quantity)||quantity<=0)throw new Error("COC_NATIVE_EDITOR_LINE_INVALID");if(!pallets.has(palletNumber))pallets.set(palletNumber,new Map());const models=pallets.get(palletNumber),key=model.replace(/[^A-Z0-9]/g,"");if(!models.has(key))models.set(key,{modelNumber:model,lots:[]});models.get(key).lots.push({model,cleanLot:lot,quantity})});return{customerName:editor.customerName.toUpperCase(),invoiceNumber:editor.invoiceNumber,ifNumber:editor.ifNumber,pallets:[...pallets.entries()].sort((left,right)=>left[0]-right[0]).map(([palletNumber,models])=>({palletNumber,modelBlocks:[...models.values()].map((block)=>({...block,totalQuantity:block.lots.reduce((sum,lot)=>sum+lot.quantity,0)}))}))}}
  async function loadPendingWorkbook(id){
    const result=await Delivery.cocWorkbookRevision("pending-revision",{deliveryId:id},branchCode());
    if(!result.pending)return loadWorkbook(id);
    const response=await fetch(result.downloadUrl,{cache:"no-store",credentials:"omit"});
    if(!response.ok)throw new Error("The saved pending COC could not be loaded. Please try again.");
    return{blob:await response.blob(),fileName:result.pending.fileName};
  }
  function returnPendingToInbox(){
    selected=null;preview=false;previewState={status:"idle",html:"",error:"",id:""};revisionState=freshRevision();
    notice={tone:"success",text:"Saved as pending in ATLAS. Open the COC and choose Edit COC to continue. Nothing was downloaded."};
    render();void loadInbox();window.scrollTo({top:0,left:0,behavior:"auto"});
  }
  async function beginNativeRevision(){if(revisionState.loading)return;const target=selected,account=receiverAccountId;revisionState.loading=true;revisionState.error="";render();try{const [workbook]=await Promise.all([loadPendingWorkbook(selected.id),Promise.resolve(Catalog?.loadRemote?.()).catch(()=>null)]),editor=await window.AtlasCocExcel.readOfficialWorkbookData(workbook.blob);if(selected!==target||receiverAccountId!==account)return;revisionState={...revisionState,step:"edit",loading:false,error:"",editor:{...editor,lines:(editor.lines||[]).map((line)=>({...line,quantityAuto:false,isNew:false,boxCount:0,unitsPerBox:0,catalogUnitsPerBox:0,boxApplied:true}))},candidate:null,filePreviewHtml:"",generatedBytes:null};render();window.scrollTo({top:0,left:0,behavior:"auto"})}catch(error){if(selected!==target||receiverAccountId!==account)return;revisionState.loading=false;revisionState.error=revisionError(error,"ATLAS could not open this COC for editing.");render()}}
  async function stageNativeRevision(form,{savePending=false}={}){const target=selected,account=receiverAccountId,warehouse=branchCode();const errorNode=form.querySelector("[data-revision-error]"),submit=form.querySelector('button[type="submit"]');syncNativeEditor();revisionState.loading=true;revisionState.error="";if(submit){submit.disabled=true;submit.textContent="GENERATING LOCKED REVIEW…"}try{const data=nativeWorkbookData({allowPending:savePending}),workbook=await loadWorkbook(selected.id),templateBytes=new Uint8Array(await workbook.blob.arrayBuffer()),bytes=await window.AtlasCocExcel.populateOfficialTemplate({templateBytes,data}),fileName=window.AtlasCocExcel.outputFileName(data.customerName,data.invoiceNumber,data.ifNumber),previewHtml=await window.AtlasCocExcel.renderOfficialWorkbookPreview(bytes);if(selected!==target||receiverAccountId!==account||branchCode()!==warehouse)return;const workbookBase64=await blobToBase64(bytes);if(selected!==target||receiverAccountId!==account||branchCode()!==warehouse)return;const result=await Delivery.cocWorkbookRevision(savePending?"save-pending":"stage-revision",{deliveryId:target.id,fileName,workbookBase64,reason:savePending?"Saved pending office completion":"Office final review correction"},warehouse);if(selected!==target||receiverAccountId!==account)return;if(savePending){returnPendingToInbox();return}revisionState={...revisionState,step:"review",loading:false,error:"",candidate:result.candidate||null,filePreviewHtml:previewHtml,generatedBytes:bytes};render();window.scrollTo({top:0,left:0,behavior:"auto"})}catch(error){if(selected!==target||receiverAccountId!==account)return;const raw=String(error?.message||"");revisionState.loading=false;revisionState.error=/COC_NATIVE_EDITOR_SKU_INCOMPLETE/.test(raw)?"Every COC row must use a complete ATLAS SKU with its color designation.":/COC_NATIVE_EDITOR_BOX_COUNT_REQUIRED/.test(raw)?"Every new row must have an applied box count.":revisionError(error,/COC_NATIVE_EDITOR_LINE_INVALID/.test(raw)?"Every COC row requires a lot number and whole-number quantity greater than zero.":"The corrected COC could not be generated.");if(errorNode)errorNode.textContent=revisionState.error;if(submit){submit.disabled=false;submit.textContent="FINISH EDITING & REVIEW"}}}
  ...
  async function approveExisting(){if(!selected||revisionState.loading)return;if(selected.status!=="OFFICE_COMPLETED"&&(!String(snapshot(selected).invoiceNumber||"").trim()||!String(snapshot(selected).ifNumber||"").trim())){revisionState.error="Enter the invoice and IF numbers using Edit COC before finalizing. This COC remains pending.";render();return}revisionState.loading=true;revisionState.error="";render();try{if(selected.status!=="OFFICE_COMPLETED"){await Delivery.markOfficeCompleted(selected.id,credentials);selected.status="OFFICE_COMPLETED";selected.office_completed_at=new Date().toISOString()}revisionState={...freshRevision(),step:"complete"};notice={tone:"success",text:"Official COC approved and saved."};void loadInbox();render();setTimeout(()=>{notice=null;render()},2600);window.scrollTo({top:0,left:0,behavior:"auto"})}catch(error){revisionState.loading=false;revisionState.error=/COC_PENDING_REVISION_REQUIRES_REVIEW/.test(error?.message||"")?"Saved edits are waiting. Choose Edit COC to review and finalize them.":/COC_FINAL_REFERENCES_REQUIRED/.test(error?.message||"")?"Add the invoice and IF numbers before finalizing.":error?.message||"The Official COC could not be approved.";render()}}
  function finishReceiverWorkflow(){const snap=snapshot(selected);completionState={customerName:snap.customerName||"",invoiceNumber:snap.invoiceNumber||""};selected=null;preview=false;previewState={status:"idle",html:"",error:"",id:""};revisionState=freshRevision();notice=null;search="";period="today";page=1;reportingDayKey=reportingDateKey();render();positionWorkflowCompletion();void loadInbox();setTimeout(()=>{completionState=null;render();window.scrollTo({top:0,left:0,behavior:"auto"})},4000)}
  async function download(button){
    ...
    try{
      ...
      if(fileHandle){
        ...
        if(button.dataset.completeWorkflow!=="true")showNotice("Official COC saved.");
      }else{
  ...
  root.addEventListener("click",async(event)=>{
    ...
    if(action==="reset-pairing"){
      if(!credentials||!window.confirm("Set up this browser as the COC Receiver? This removes only its saved pairing. It does not revoke another office computer or delete any COC. A supervisor must approve the new pairing."))return;
      const current=credentials,authSequence=receiverAuthSequence,key=`office-coc-receiver-credentials:${current.stationKey}`;
      try{
        for(const settingKey of [key,...(current.warehouseCode==="CA"?["office-coc-receiver-credentials"]:[])]){
          const stored=await window.AtlasCocStorage.getSetting(settingKey);
          if(current!==credentials||authSequence!==receiverAuthSequence)return;
          if(settingKey===key&&stored?.value&&stored.value.devicePublicId!==current.devicePublicId)throw new Error("Pairing changed in this browser. Reload and try again.");
          if(stored?.value?.devicePublicId===current.devicePublicId)await window.AtlasCocStorage.deleteSetting(settingKey);
        }
        if(current!==credentials||authSequence!==receiverAuthSequence)return;
        ++receiverAuthSequence;authorizationPending=false;clearTimeout(authorizationRetryTimer);clearReceiverAccount();receiverAuthState="unpaired";render();await startPairing();
      }catch(error){if(current===credentials&&authSequence===receiverAuthSequence)showNotice(error?.message||"Could not reset this browser pairing.","error")}
      return;
    }
    ...
    if(action==="save-pending"){const form=root.querySelector("[data-native-editor]");if(!revisionState.loading&&form&&validateNativeEditor(form,{allowPending:true}))await stageNativeRevision(form,{savePending:true})}
    if(action==="revision-cancel"){if(revisionState.loading)return;revisionState={...revisionState,step:"preview",loading:false,error:"",editor:null,candidate:null,filePreviewHtml:"",generatedBytes:null};render();positionOfficialPreview()}
  ...
  function clearReceiverAccount(){
    ...
    previewState={status:"idle",html:"",error:"",id:""};revisionState=freshRevision();notice=null;branchContext=null;inboxLoaded=false;inboxFailed=false;
