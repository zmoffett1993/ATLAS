(function(){
  ...
  function newEditorLine(palletNumber){return{palletNumber,model:"",lot:"",quantity:"",quantityAuto:false}}
  function nativePalletTotal(lines){return lines.reduce((sum,line)=>sum+(Number(line.quantity)||0),0)}
  function nativeEditorLineMarkup(line,pallet){const listId=`receiver-sku-list-${line.index}`,selectedSku=completeSkuRecord(line.model)?.modelNumber||"";return `<article class="receiver-edit-sheet-row" data-native-line data-line-index="${line.index}" data-selected-sku="${esc(selectedSku)}" data-quantity-auto="${line.quantityAuto?"true":"false"}"><input type="hidden" name="palletNumber" value="${pallet}"><div class="receiver-edit-cell receiver-edit-model-cell"><span class="sr-only">Complete model number with color</span><input name="model" maxlength="100" autocomplete="off" autocapitalize="characters" spellcheck="false" value="${esc(line.model)}" placeholder="Select complete SKU" aria-label="Complete model number with color on pallet ${pallet}" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="${listId}" data-receiver-sku-input required><i class="receiver-sku-chevron" aria-hidden="true">⌄</i><div id="${listId}" class="receiver-sku-suggestions" role="listbox" hidden></div></div><label class="receiver-edit-cell receiver-edit-lot-cell"><span class="sr-only">Lot number</span><input name="lot" maxlength="160" autocomplete="off" autocapitalize="characters" value="${esc(line.lot)}" placeholder="Enter lot number" aria-label="Lot number on pallet ${pallet}" required></label><label class="receiver-edit-cell receiver-edit-quantity-cell"><span class="sr-only">Quantity</span><input name="quantity" type="number" inputmode="numeric" min="1" step="1" value="${Number(line.quantity)||""}" placeholder="0" aria-label="Quantity for ${esc(line.model||`pallet ${pallet}`)}" required></label><button type="button" class="receiver-native-remove" data-action="native-remove-line" data-index="${line.index}" aria-label="Remove this COC row" title="Remove row">×</button>${line.quantityAuto?`<small class="receiver-native-autofill">ATLAS quantity · editable</small>`:""}</article>`}
  ...
  function clearReceiverFieldError(input){
    ...
    input.closest(".receiver-edit-cell, label")?.classList.remove("is-invalid");
  }
  function markReceiverFieldError(input,message){
    input.setCustomValidity(message);
    input.setAttribute("aria-invalid","true");
    input.classList.add("is-invalid");
    input.closest(".receiver-edit-cell, label")?.classList.add("is-invalid");
  }
  function revealReceiverField(input){
    const target=input.closest(".receiver-edit-cell, label")||input;
  ...
  function validateNativeEditor(form){
    ...
    form.querySelectorAll("[data-native-line]").forEach((row)=>{
      fields.push(
        ...
        [row.querySelector('[name="quantity"]'),"Enter a whole-number quantity greater than zero.","quantity"],
      );
    });
    let firstInvalid=null;
    fields.forEach(([input,message,type])=>{
      if(!input)return;
      const value=String(input.value||"").trim();
      const invalid=!value||(type==="sku"&&!completeSkuRecord(value))||(type==="quantity"&&(!Number.isSafeInteger(Number(value))||Number(value)<=0));
