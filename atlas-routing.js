(() => {
  ...
  const state = { planningGeneration: 0, planningController: null, planned: null, loads: [], lockedTrips: [], nextLoadPriority: null, assignments: {}, vanConfirmed: {}, truckPalletTarget: 11, dailyTripTarget: 3, open: false, ownerId: null, catalog: [], orders: [], draftPhotos: [], newUrls: [], allUrls: new Set(), editId: null, drag: null, importGeneration: 0 };
  ...
  let driverDirectory = null;
  const assignmentAPI = () => window.atlasRoutingAssignment;
  function defaultAssignment() {
    if (!driverDirectory) return "Bubba:truck";
    const candidates = driverDirectory.candidates;
    const matches = driverDirectory.defaultDriverUserId ? candidates.filter(c=>c.userId===driverDirectory.defaultDriverUserId) : candidates.filter(c=>c.displayName.trim().toLowerCase()==="bubba");
    return matches.length===1 ? {driverUserId:matches[0].userId,driverName:matches[0].displayName,vehicleId:"box_truck",scheduleId:matches[0].scheduleId} : "Bubba:truck";
  }
  const assignmentAt = index => state.assignments[index] || defaultAssignment();
  const assignmentVan = value => assignmentAPI().read(value).vehicleId !== 'box_truck';
  function assignmentControls(index,locked=false) {
    const value=assignmentAt(index), a=assignmentAPI().read(value);
    if(!driverDirectory && typeof value==='string')return '';
    const candidates=assignmentAPI().sorted(driverDirectory?.candidates || []);
    const names=new Map();for(const c of candidates)names.set(c.displayName,(names.get(c.displayName)||0)+1);
    const options=candidates.map(c=>'<option value="'+escape(c.userId)+'" '+(c.userId===a.driverUserId?'selected':'')+'>'+escape(c.displayName)+(names.get(c.displayName)>1?' · '+escape(c.homeWarehouseCode)+' · '+escape(c.userId.slice(-6)):'')+'</option>').join('');
    return '<div class="atlas-driver-assignment-controls"><label>Driver<select data-route-driver="'+index+'" '+(locked||accessReadOnly?'disabled':'')+'>'+(!a.driverUserId||!candidates.some(c=>c.userId===a.driverUserId)?'<option value="">'+escape(a.driverName)+' — review account</option>':'')+options+'</select></label><label>Vehicle<select data-route-vehicle="'+index+'" '+(locked||accessReadOnly?'disabled':'')+'>'+Object.entries(assignmentAPI().vehicles).map(([id,label])=>'<option value="'+id+'" '+(a.vehicleId===id?'selected':'')+'>'+label+'</option>').join('')+'</select></label><label>Planning schedule<select data-route-schedule="'+index+'" '+(locked||accessReadOnly?'disabled':'')+'><option value="standard" '+(a.scheduleId==='standard'?'selected':'')+'>6 AM–3 PM · lunch after final return</option><option value="relief" '+(a.scheduleId==='relief'?'selected':'')+'>8 AM–5 PM · midday lunch</option></select></label><small>Planning assumption. Confirm the employee can drive this vehicle.</small>'+(!accessReadOnly?'<button type="button" class="atlas-route-button" data-route-publish-driver="'+index+'">Publish assigned trip</button>':'')+'</div>';
  }
  async function loadDriverDirectory() {
    const owner=state.ownerId,entry=entryGeneration;
    try {const result=await assignmentAPI().request('candidates');if(state.open&&owner===state.ownerId&&entry===entryGeneration)driverDirectory=result;}
    catch { /* Existing saved assignments remain untouched if directory is unavailable. */ }
  }

  let scannerStartup = null, scannerOwner = null, deleteRequest = null, deletingOrder = false;
  const dispatchUI = { timing: "pending", tab: "orders", query: "", move: null, lastPlan: null };
  ...
  const iconPaths = {
    pencil: '<path d="m4 16 12-12 4 4-12 12-5 1zM14 6l4 4"/>',
  ...
  function mount() {
    ...
    section.innerHTML = `
      <div class="atlas-route-scanner-opening" aria-live="polite"><header><button type="button" data-scanner-cancel aria-label="Back to Home">←</button>ATLAS · Order Scanner</header><div>${icon("camera")}<h1>Scan Delivery Order</h1><p data-scanner-startup-status>Opening your scanner…</p><span class="atlas-scanner-spinner" aria-hidden="true"></span><div data-scanner-recovery hidden><button type="button" class="atlas-route-button atlas-route-primary" data-scanner-retry>Try Again</button><button type="button" class="atlas-route-button" data-scanner-photos>Choose from Photos</button></div><small>Preparing camera and secure order upload</small></div></div>
      ...
      <main class="atlas-route-main">
        <header class="atlas-route-header">
          <div><button type="button" class="atlas-route-button" data-route-host-menu>ATLAS Menu</button><h1>Daily Route Optimizer</h1><p>Plan and optimize today’s deliveries with intelligent routing and vehicle capacity.</p></div>
          <div class="atlas-route-header-actions"><label class="atlas-route-date">${icon("calendar")}<span>Planning day</span> <input type="date" data-route-date /></label><button type="button" class="atlas-route-button" data-route-history>${icon("calendar")}Previous Trips</button><button type="button" class="atlas-route-account" data-route-account aria-label="ATLAS account"><span class="atlas-route-avatar" data-route-avatar></span><span><strong data-route-user></strong><small>Delivery planning</small></span>${icon("chevron")}</button></div>
        </header>
        ...
        <div class="atlas-route-grid">
          <div class="atlas-route-column">
            ...
            ${card(`${icon("document")} Today's Deliveries`, `<div class="atlas-route-card-tools atlas-route-order-tools"><button type="button" class="atlas-route-button atlas-route-primary" data-route-intake>${icon("camera")}Add Orders</button><button type="button" class="atlas-route-button atlas-route-primary atlas-route-optimize" data-route-optimize disabled title="Scan orders on mobile and connect routing to calculate traffic and timing">${icon("play")}Optimize Routes</button></div>
    ...
    initializeMobileDashboard(section);
    const deletion = document.createElement("dialog"); deletion.className = "atlas-route-intake atlas-route-delete-dialog"; deletion.dataset.routeDeleteDialog = ""; deletion.setAttribute("aria-labelledby", "atlasRouteDeleteTitle"); section.append(deletion);
    deletion.addEventListener("cancel", event => { if (deletingOrder) event.preventDefault(); });
    deletion.addEventListener("close", () => { deleteRequest = null; });
    ...
    section.addEventListener("click", (event) => {
      ...
      if (event.target.closest("[data-route-driver-admin]")) void window.atlasDriverWorkspace?.manage(find("[data-route-date]").value);
      if (event.target.closest("[data-route-publish-driver]")) void publishDriverTrip(Number(event.target.closest("[data-route-publish-driver]").dataset.routePublishDriver));
      ...
      if (event.target.closest("[data-route-delete]")) requestDeleteOrder(event.target.closest("[data-route-delete]").dataset.routeDelete);
      if (event.target.closest("[data-route-delete-confirm]")) void confirmDeleteOrder();
      if (event.target.closest("[data-route-delete-cancel]")) find("[data-route-delete-dialog]").close();
      if (event.target.closest("[data-scanner-cancel]")) cancelScannerStartup();
      if (event.target.closest("[data-scanner-retry]")) void startScannerIntent("retry");
      if (event.target.closest("[data-scanner-photos]")) void startScannerIntent("photos");
    ...
    section.addEventListener("change", (event) => {
      if(event.target.matches("[data-route-driver],[data-route-vehicle],[data-route-schedule]")){
        const index=Number(event.target.dataset.routeDriver ?? event.target.dataset.routeVehicle ?? event.target.dataset.routeSchedule);
        if(accessReadOnly||index<state.lockedTrips.length)return;
        const a=assignmentAPI().read(assignmentAt(index));
        if(event.target.hasAttribute("data-route-driver")){const candidate=driverDirectory?.candidates.find(c=>c.userId===event.target.value);if(!candidate)return;a.driverUserId=candidate.userId;a.driverName=candidate.displayName;a.scheduleId=candidate.scheduleId;}
        if(event.target.hasAttribute("data-route-vehicle"))a.vehicleId=event.target.value;
        if(event.target.hasAttribute("data-route-schedule"))a.scheduleId=event.target.value;
        if(!a.driverUserId){window.alert("Choose a driver account first.");return;}
        state.assignments[index]=assignmentAPI().validate({driverUserId:a.driverUserId,driverName:a.driverName,vehicleId:a.vehicleId,scheduleId:a.scheduleId});state.vanConfirmed[index]=false;
        renderOrders({keepAssignments:true});return;
      }
  ...
  function initializeDispatchUI(section) {
    ...
    map.innerHTML = '<button type="button" class="atlas-route-close" data-dispatch-close="map" aria-label="Close map">×</button><h2>Route map</h2>';
    ...
    filter.innerHTML = '<span>Find an order</span><input type="search" data-dispatch-query placeholder="Search by customer, city, SO, or invoice…" />';
    orders.querySelector(".atlas-route-table-scroll").before(filter);
    const count = document.createElement("p"); count.className = "atlas-dispatch-order-count"; count.dataset.routeVisibleCount = ""; orders.append(count);
    ...
    const remaining = document.createElement("details"); remaining.className = "atlas-route-remaining"; remaining.dataset.routeRemaining = "";
    remaining.innerHTML = '<summary>Remaining Plan <small data-route-remaining-summary></small></summary>';
    remaining.append(find("[data-route-plan]")); plan.querySelector("h2").remove(); plan.append(remaining);
    ...
    section.addEventListener("click", (event) => {
      ...
      const show = event.target.closest("[data-dispatch-show]"); if (show) openRoutingDrawer(show.dataset.dispatchShow);
      if(event.target.closest("[data-route-retry-arrivals]"))void optimizeDay();
      const close = event.target.closest("[data-dispatch-close]"); if (close && !(close.dataset.dispatchClose === "settings" && savedDay.busy)) find(`[data-dispatch-${close.dataset.dispatchClose}]`)?.close();
    ...
    section.addEventListener("click", (event) => {
      const vehicle = event.target.closest("[data-next-vehicle]"); if(vehicle){find("[data-route-remaining]").open=true;find(`[data-route-assignment="${vehicle.dataset.nextVehicle}"]`)?.focus();}
      const priority = event.target.closest("[data-next-priority]"); if (priority) openNextPriority(priority.dataset.nextPriority);
      const confirm = event.target.closest("[data-next-confirm]"); if (confirm) applyNextPriority(confirm.dataset.nextConfirm);
      if(event.target.closest("[data-next-capacity]")){find("[data-dispatch-move]").close();openRoutingDrawer("settings");}
    });
  ...
  function applyDispatchTab() {
    ...
    find(".atlas-route-header h1").textContent = driverMode ? "My Deliveries" : dispatchUI.tab === "pods" ? "Delivery Documents" : "Delivery Routing";
    if (dispatchUI.tab === "trips") find("[data-route-remaining]").open = true;
    ...
    section.querySelectorAll("[data-manager-tab]").forEach(button => {
      const active = button.dataset.managerTab === (dispatchUI.tab === "orders" ? "home" : dispatchUI.tab);
  ...
  function filterDispatchOrders() {
    ...
    find("[data-route-mobile-list]")?.querySelectorAll("[data-route-row]").forEach(row => {
      row.hidden = !!q && !row.dataset.routeMobileSearch.includes(q);
    });
    const rows = [...(find("[data-route-orders]")?.querySelectorAll("[data-route-row]") || [])];
    if (find("[data-route-visible-count]")) find("[data-route-visible-count]").textContent = `Showing ${rows.filter(row => !row.hidden).length} of ${rows.length} deliveries`;
    const input = find("[data-route-mobile-query]");
    if (input && input.value !== dispatchUI.query) input.value = dispatchUI.query;
    if (q && find(".atlas-route-mobile-search")?.hidden) {
      find(".atlas-route-mobile-search").hidden = false;
      find('[data-route-mobile-action="search"]').setAttribute("aria-expanded", "true");
    }
  }

  // Mobile projects the existing day; commands and persistence remain shared.
  function initializeMobileDashboard(section) {
    const mobile = document.createElement("section"); mobile.className = "atlas-route-mobile";
    mobile.innerHTML = `<header class="atlas-route-mobile-header"><img src="./atlas-brand-landscape-dark.svg?v=128" alt="ATLAS"/><button type="button" data-route-mobile-action="more" aria-label="More day actions">⋮</button><h1>Today's Routes</h1></header>
      <div class="atlas-route-mobile-body"><div class="atlas-route-mobile-datebar"><label>${icon("calendar")}<span data-route-mobile-date-label></span><input type="date" data-route-mobile-date aria-label="Planning date"/></label><button type="button" data-route-mobile-action="save" data-route-mobile-save aria-live="polite"></button></div>
      <section class="atlas-route-mobile-summary"><div><small>ROUTE SETUP</small><h2 data-route-mobile-headline></h2><p data-route-mobile-totals></p></div><span class="atlas-route-mobile-illustration" aria-hidden="true">${icon("route")}</span><div class="atlas-route-mobile-actions"><button type="button" class="atlas-route-mobile-primary" data-route-mobile-action="add">${icon("camera")}Add Orders</button><button type="button" class="atlas-route-mobile-primary" data-route-mobile-action="optimize">${icon("play")}Optimize Routes</button></div></section>
      <div class="atlas-route-mobile-list-heading"><h2>Deliveries <span data-route-mobile-count></span></h2><button type="button" class="atlas-route-mobile-search-toggle" data-route-mobile-action="search" aria-label="Search deliveries" aria-expanded="false"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="10" cy="10" r="7"/><path d="m15 15 6 6"/></svg></button></div>
      <label class="atlas-route-mobile-search" hidden>Find a delivery<input type="search" data-route-mobile-query placeholder="Customer, city, SO, invoice or fulfillment"/></label><div class="atlas-route-mobile-list" data-route-mobile-list></div></div>`;
    find(".atlas-route-main").prepend(mobile);
    const reorder = document.createElement("dialog"); reorder.className = "atlas-route-manager-more"; reorder.dataset.routeMobileReorderDialog = ""; reorder.setAttribute("aria-label", "Reorder delivery"); section.append(reorder);
    mobile.addEventListener("click", event => {
      const id = event.target.closest("[data-route-mobile-reorder]")?.dataset.routeMobileReorder;
      if (!id || !canReorderOrder(id)) return;
      const orders = dayOrders(), index = orders.findIndex(o => o.id === id);
      reorder.innerHTML = `<h2>Move delivery</h2><p>${escape(orders[index].customer)}</p>${[[-1,"Move earlier"],[1,"Move later"]].map(([step,label]) => `<button type="button" class="atlas-route-button" data-route-move="${step}" data-id="${escape(id)}" ${orders[index+step] && canReorderOrder(orders[index+step].id) ? "" : "disabled"}>${label}</button>`).join("")}<button type="button" class="atlas-route-button" data-route-mobile-close-reorder>Cancel</button>`;
      reorder.showModal();
    });
    reorder.addEventListener("click", event => { if (event.target.closest("button:not(:disabled)")) reorder.close(); });
    mobile.addEventListener("change", event => {
      if (!event.target.matches("[data-route-mobile-date]")) return;
      const date = find("[data-route-date]"); if (date.disabled) return;
      date.value = event.target.value; date.dispatchEvent(new Event("change")); renderMobileDashboard();
    });
    mobile.addEventListener("input", event => {
      if (!event.target.matches("[data-route-mobile-query]")) return;
      dispatchUI.query = event.target.value; find("[data-dispatch-query]").value = dispatchUI.query; filterDispatchOrders();
    });
    mobile.addEventListener("click", async event => {
      const button = event.target.closest("[data-route-mobile-action]"); if (!button || button.disabled) return;
      const action = button.dataset.routeMobileAction;
      if (action === "more") find('[data-manager-tab="more"]').click();
      if (action === "save") find("[data-route-save-day]").click();
      if (action === "add") find("[data-route-intake]").click();
      if (action === "search") {
        const field = find(".atlas-route-mobile-search"); field.hidden = !field.hidden;
        button.setAttribute("aria-expanded", String(!field.hidden)); button.setAttribute("aria-label", field.hidden ? "Search deliveries" : "Close search");
        if (field.hidden) { dispatchUI.query = ""; find("[data-route-mobile-query]").value = find("[data-dispatch-query]").value = ""; filterDispatchOrders(); }
        else find("[data-route-mobile-query]").focus();
      }
      if (action === "optimize" && !find("[data-route-optimize]").disabled) {
        // Reveal the existing progress, stop-calculation and trip controls.
        dispatchUI.tab = "trips"; applyDispatchTab(); window.scrollTo(0,0); await optimizeDay();
      }
    });
  }

  function openRoutingDrawer(kind) {
    if (!["map", "settings"].includes(kind)) return;
    const dialog=find(`[data-dispatch-${kind}]`);
    if(!dialog || savedDay.busy) return;
    const trigger=document.activeElement, scroll=window.scrollY;
    document.querySelectorAll(".atlas-dispatch-map[open],.atlas-dispatch-settings[open]").forEach(d=>d.close());
    dialog.setAttribute("aria-label",kind==="map"?"Route map":"Route settings");
    if(kind==="settings") prepareSettingsDrawer(dialog);
    dialog.showModal();
    document.querySelectorAll("[data-dispatch-show]").forEach(b=>b.setAttribute("aria-pressed",String(b.dataset.dispatchShow===kind)));
    dialog.addEventListener("close",()=>{
      document.querySelectorAll("[data-dispatch-show]").forEach(b=>b.setAttribute("aria-pressed","false"));
      trigger?.focus?.({preventScroll:true}); window.scrollTo(0,scroll);
    },{once:true});
    if(kind==="map") requestAnimationFrame(()=>{
      const connection=window.atlasRoutingConnection;
      const plan=state.planned, owner=state.ownerId;
      const current=()=>dialog.open&&state.ownerId===owner&&state.planned===plan;
      const status=dialog.querySelector("[data-map-status]")||Object.assign(document.createElement("p"),{className:"atlas-route-intake-note"});
      status.dataset.mapStatus="";dialog.append(status);status.textContent="Loading map…";
      if(!connection?.available){status.textContent="Map unavailable. Connect Delivery Routing, then reopen the map.";return;}
      Promise.resolve(plan?connection.drawRoutes(find("[data-route-map]"),plan,current):connection.showMap(find("[data-route-map]"))).then(()=>{
        if(current()){window.dispatchEvent(new Event("resize"));status.textContent=plan?"":"Calculate routes to see your trip paths.";}
      }).catch(()=>{if(current()){status.textContent="Map unavailable. ";const retry=document.createElement("button");retry.className="atlas-route-button";retry.textContent="Retry";retry.onclick=()=>openRoutingDrawer("map");status.append(retry);}});
    });
  }

  function prepareSettingsDrawer(dialog) {
    const catalog=dialog.querySelector(".atlas-route-catalog-bar");
    if(catalog)dialog.append(catalog);
    dialog.querySelector("[data-settings-form]")?.remove();
    for(const el of [...dialog.children]) if(el.matches(".atlas-route-card,.atlas-route-plan-settings,.atlas-route-help"))el.hidden=true;
    const form=document.createElement("form");form.dataset.settingsForm="";
    const settings=dayDocument().settings;
    form.innerHTML=`<section><h3>Drivers and schedule</h3><button type="button" class="atlas-route-button" data-route-driver-admin>Driver accounts &amp; test routes</button><p><strong>Bubba · Primary driver</strong><br>6:00 AM–3:00 PM · Box truck / van</p><p>Lunch: <strong>After final return · 60 minutes</strong></p><p><strong>Achmad · Relief driver</strong><br>8:00 AM–5:00 PM · Cargo van</p><label>Achmad lunch starts<input name="lunch" type="time" min="11:00" max="13:00" required></label></section><section><h3>Load planning</h3><div class="atlas-settings-grid"><label>Truck pallet target<input name="truckPalletTarget" type="number" min="1" step="1" required></label><label>Typical trips<input name="dailyTripTarget" type="number" min="1" step="1" required></label><label>Reload time (minutes)<input name="reloadMinutes" type="number" min="0" max="120" step="1" required></label></div><label class="atlas-settings-check"><input name="preserveOrder" type="checkbox">Keep my stop order</label><details><summary>Vehicle details</summary><p>Box truck: 12 pallet spaces; usually 11 with the pallet jack. The target is adjustable.</p><p>Two matching Nissan NV vans: 2 pallets each or loose boxes, with an 80% volume planning target.</p></details></section><section><h3>Warehouse</h3><p><strong>Chubby Gorilla</strong><br>${escape(DEPOT)}</p></section><details><summary>Advanced</summary><div data-settings-catalog></div><p>NEW product specifications override older duplicates. Missing dimensions require warehouse review.</p></details><p role="status" data-settings-status></p><footer><button type="button" class="atlas-route-button" data-dispatch-close="settings">Cancel</button><button type="submit" class="atlas-route-button atlas-route-primary">Save settings</button></footer>`;
    for(const [key,value] of Object.entries(settings))if(form.elements[key]){if(key==="preserveOrder")form.elements[key].checked=value;else form.elements[key].value=value;}
    if(catalog){form.querySelector("[data-settings-catalog]").append(catalog);catalog.querySelector("[data-route-catalog-status]").textContent=`${state.catalog.length} product specifications`;}
    dialog.append(form);
    form.querySelectorAll("input,button[type=submit]").forEach(el=>el.disabled=accessReadOnly||driverMode||!savedDay.canEdit);
    form.addEventListener("submit",async event=>{
      event.preventDefault();if(accessReadOnly||driverMode||savedDay.busy||!storage()?.enabled)return;
      const owner=state.ownerId,generation=savedDay.generation,before=JSON.stringify(dayDocument()),revision=savedDay.revision;
      const values=Object.fromEntries(new FormData(form));values.preserveOrder=form.elements.preserveOrder.checked;
      for(const key of ["truckPalletTarget","dailyTripTarget","reloadMinutes"])values[key]=Number(values[key]);
      let doc;try{doc=window.atlasRoutingStorage.document({...dayDocument(),settings:values,catalog:form.catalogDraft||state.catalog});}catch(e){form.querySelector("[data-settings-status]").textContent=e.message;return;}
      savedDay.busy=true;form.inert=true;form.querySelector("[data-settings-status]").textContent="Saving settings…";
      const current=()=>owner===state.ownerId&&generation===savedDay.generation&&dialog.open;
      const preventClose=e=>e.preventDefault();dialog.addEventListener("cancel",preventClose);
      try{
        const result=await storage().save(doc.date,revision,doc);if(!current())return;
        if(before!==JSON.stringify(dayDocument()))throw Error("The day changed while saving. Reopen the saved day before editing again.");
        for(const [key,selector] of [["truckPalletTarget","truck-target"],["dailyTripTarget","trip-target"],["reloadMinutes","reload"],["lunch","lunch"],["preserveOrder","preserve"]]){
      ...
      if(owner===state.ownerId&&!dialog.open&&!state.lockedTrips.length&&window.atlasRoutingConnection?.available)await optimizeDay();
    });
  }

  function renderMobileDashboard() {
    const mobile = find(".atlas-route-mobile"); if (!mobile) return;
    const orders = dayOrders(), analyzed = state.analyzed || [], plan = state.loadPlan;
    const date = find("[data-route-date]");
    find("[data-route-mobile-date]").value = date.value; find("[data-route-mobile-date]").disabled = date.disabled;
    find("[data-route-mobile-date-label]").textContent = date.value ? new Date(date.value + "T12:00:00").toLocaleDateString("en-US", {month:"short",day:"numeric",year:"numeric"}) : "Choose date";
    const save = find("[data-route-mobile-save]"), enabled = storage()?.enabled;
    const label = !enabled ? "Temporary session" : savedDay.busy ? "Working…" : !savedDay.ready ? "Unavailable" : mobileSaveFailed ? "Save failed · Retry" : savedDay.dirty ? "Save changes" : savedDay.revision ? "✓ Saved" : "Not saved";
    save.textContent = label; save.dataset.state = label === "✓ Saved" ? "saved" : "review";
    save.disabled = find("[data-route-save-day]").disabled || (!savedDay.dirty && !!savedDay.revision);
    save.title = find("[data-route-save-status]").textContent;
    if (savedDay.busy && savedDay.message.startsWith("Saving")) save.textContent = "Saving…";
    find("[data-route-mobile-headline]").textContent = orders.length ? `${orders.length} ${orders.length === 1 ? "delivery" : "deliveries"} ready` : "No deliveries yet";
    const needsReview = analyzed.some(o => o.palletSpaces == null) || !!plan?.unscheduled.length;
    const pallets = analyzed.reduce((n,o) => n + (o.palletSpaces || 0),0), trips = plan?.trips.length || 0;
    find("[data-route-mobile-totals]").textContent = needsReview ? "Review required" : `${pallets} pallet${pallets === 1 ? "" : "s"} · Estimated ${trips} trip${trips === 1 ? "" : "s"}`;
    find('[data-route-mobile-action="optimize"]').disabled = find("[data-route-optimize]").disabled || !!state.planningController;
    find('[data-route-mobile-action="add"]').disabled = !intakeSnapshot().canEdit;
    find("[data-route-mobile-count]").textContent = String(orders.length);
    const html = analyzed.map(result => {
      const o = result.source, test = /\b(?:test delivery|driver test|synthetic|not a real delivery)\b/i.test(`${o.customer} ${o.notes}`);
      const status = isLockedOrder(o.id) && !o.dispatchedOn ? "Partially sent out" : window.atlasRoutingStorage.deliveryStatus(o).label;
      const badge = test ? "TEST DELIVERY" : result.issues.length ? "Review required" : isLockedOrder(o.id) || o.dispatchedOn || o.deliveredOn || o.deliveryException ? status : "";
      const query = [o.customer,o.city,o.orderNumber,...(o.invoiceNumbers || []),...(o.fulfillmentNumbers || [])].join(" ").toLowerCase();
      return `<article class="atlas-route-mobile-order" data-route-row="${escape(o.id)}" data-route-mobile-search="${escape(query)}"><button type="button" class="atlas-route-mobile-grip" data-route-mobile-reorder="${escape(o.id)}" aria-label="Reorder ${escape(o.customer)}" ${canReorderOrder(o.id) ? "" : "disabled"}>⠿</button><button type="button" class="atlas-route-mobile-open" data-route-edit="${escape(o.id)}" ${accessReadOnly || savedDay.busy ? "disabled" : ""}><span class="atlas-route-mobile-order-info"><strong>${escape(o.customer)}</strong>${badge ? `<small class="atlas-route-mobile-badge">${escape(badge)}</small>` : ""}<span class="atlas-route-mobile-number">${escape(o.orderNumber)}</span><span>${icon("pin")}${escape(o.city)}</span>${o.timeWindow ? `<span>${icon("clock")}${escape(o.timeWindow)}</span>` : ""}</span><span class="atlas-route-mobile-pallets"><b>${result.palletSpaces ?? "—"}</b><span>${result.palletSpaces == null ? "Review" : result.palletSpaces === 1 ? "pallet" : "pallets"}</span></span>${icon("chevron")}</button>${orderActions(o)}</article>`;
    }).join("");
    const list = find("[data-route-mobile-list]"); if (list.dataset.rendered !== html) { list.innerHTML = html; list.dataset.rendered = html; }
    filterDispatchOrders();
    const detail = find("[data-route-mobile-save-detail]"); if (detail) detail.textContent = save.title;
    for (const [action, selector] of Object.entries(mobileDayActions)) {
      const button = find(`[data-manager-more="${action}"]`); if (button) button.disabled = !!find(selector)?.disabled;
    }
  }

  const mobileDayActions = {save:"[data-route-save-day]",history:"[data-route-history]",load:"[data-route-load-day]",reminders:"[data-route-reminders]",confirm:"[data-route-confirm-day]"};

  function dispatchRows(shipments, { queue = false, compact = false } = {}) {
    return `<div class="atlas-route-table-scroll atlas-dispatch-load-table"><table><tbody>${shipments.map(shipment => {
      ...
      return `<tr data-route-row="${order.id}"><td class="atlas-route-order-position">${editable ? `<button type="button" data-route-drag="${order.id}" aria-label="Hold to move ${escape(order.customer)}">⠿</button>` : '<span aria-label="Sent-out load locked">●</span>'}</td><td><button type="button" class="atlas-route-order-link" data-route-edit="${order.id}">${escape(order.customer)}</button><small>${escape(order.orderNumber)} · ${escape(order.city)}</small><small class="${compact ? "atlas-next-secondary" : ""}">${escape(date)} · Trip ${shipment.tripIndex + 1}${tripIndices.length > 1 ? ` · Split: ${tripIndices.length} shipments` : ""}</small><small class="atlas-arrival-status">${state.planningController ? "Arrival updating…" : visit && Number.isFinite(Date.parse(visit.arrival)) && dispatchUI.timing !== "failed" ? `Expected arrival: ${displayTime(visit.arrival)}` : dispatchUI.timing === "failed" ? "Arrival unavailable" : "Arrival pending"}${dispatchUI.timing === "failed" ? ' <button type="button" class="atlas-route-button" data-route-retry-arrivals>Retry</button>' : ""}</small>${order.timeWindow ? `<small>Receiving hours: ${escape(order.timeWindow)}</small>` : ""}${order.checkOnDelivery ? '<small class="atlas-route-check-badge">CHECK ON DELIVERY</small>' : ""}${order.notes ? `<small>${escape(order.notes)}</small>` : ""}<div class="atlas-dispatch-row-actions">${compact ? priorityAction(order) : ""}${!compact && editable ? `${queue ? `<button type="button" class="atlas-route-button" data-dispatch-promote="${order.id}">Move to next load</button>` : ""}<button type="button" class="atlas-route-button" data-dispatch-choose="${order.id}">Move…</button>` : !editable ? '<small>Locked or read-only</small>' : ""}</div></td><td></td><td><strong>${shipment.palletSpaces ?? "—"}</strong></td><td></td><td></td></tr>`;
  ...
  function renderDispatch() {
    ...
    const target = state.truckPalletTarget;
    const loadCard = (trip, index, following = false) => {
      const assignment = assignmentAt(index), isVan = assignmentVan(assignment);
      const fit = isVan ? core.assessVanShipments(trip.shipments, state.catalog) : null;
      const atTarget = !isVan && trip.palletSpaces >= target;
      const percent = Math.round(trip.palletSpaces / target * 100);
      return `<section class="atlas-next-load-section" data-route-trip-drop="${index}" ${following ? "" : "data-next-load-primary"}><header><h2>${icon(isVan ? "van" : "truck")}Trip ${index + 1} · ${following ? "Building next load" : "Next load"}</h2><button type="button" class="atlas-next-assignment" data-next-vehicle="${index}" aria-label="Review driver and vehicle for trip ${index+1}">${icon(isVan ? "van" : "truck")}${escape(assignmentAPI().read(assignment).driverName + " · " + assignmentAPI().vehicles[assignmentAPI().read(assignment).vehicleId])}</button></header>${assignmentControls(index)}
        <div class="atlas-dispatch-load-heading"><strong>${isVan ? `${trip.palletSpaces} pallet equivalents` : `${trip.palletSpaces} / ${target} target pallets`}</strong>${atTarget ? '<span class="atlas-route-ready">At planning target</span>' : ""}</div>
        ${isVan ? `<p class="atlas-dispatch-muted">${escape(fit.reason)}${fit.percent == null ? "" : ` · ${fit.percent}%`}</p>` : `<div class="atlas-next-meter"><meter min="0" max="${target}" value="${trip.palletSpaces}" aria-label="Trip ${index + 1} planning target"></meter><b>${percent}%</b></div><p class="atlas-dispatch-muted">${Math.max(0,target-trip.palletSpaces)} pallet spaces available · planning target</p>${atTarget ? '<p class="atlas-next-fit-note">Confirm load fit with warehouse.</p>' : ""}`}
        <p class="atlas-dispatch-muted">${trip.shipments.length} stop${trip.shipments.length===1?'':'s'} on this load</p>
        ${dispatchRows(trip.shipments.map(shipment => ({...shipment,tripIndex:index})),{compact:true})}
        ${!following ? '<div class="atlas-next-drop" data-route-next-drop><strong>Drag a delivery here to prioritize</strong><span data-route-drop-preview>Or use the order’s load actions.</span></div>' : ""}</section>`;
    };
    find("[data-dispatch-next]").innerHTML = next ? loadCard(next,nextIndex) + (next.palletSpaces >= target && !assignmentVan(assignmentAt(nextIndex)) && state.loads[nextIndex+1] ? loadCard(state.loads[nextIndex+1],nextIndex+1,true) : "") : '<h2>Next load</h2><p class="atlas-dispatch-muted">No unsent load. Scan delivery orders on mobile to begin.</p>';
    find("[data-dispatch-next]").dataset.routeTripDrop = String(nextIndex);
    const unresolved = state.loadPlan?.unscheduled || [];
    if (unresolved.length) find("[data-dispatch-next]").insertAdjacentHTML("beforeend", `<div class="atlas-route-planning-review">${unresolved.map(item=>`<p>${escape(item.customer)} · ${escape(item.reason)}</p>`).join("")}</div>`);
    find("[data-route-remaining-summary]").textContent = `${state.loads.length} trips · ${state.loads.slice(nextIndex+1).reduce((sum,trip)=>sum+trip.shipments.length,0)} later shipments · ${unresolved.length} unresolved${state.loadPlan?.warnings.length ? ` · ${state.loadPlan.warnings.length} review notes` : ""}`;
  ...
  function priorityAction(order) {
    const pinned=state.nextLoadPriority?.includes(order.id), onNext=state.loads[state.lockedTrips.length]?.shipments.some(s=>s.orderId===order.id);
    const action=label=>`<button type="button" class="atlas-route-button atlas-next-priority" data-next-priority="${escape(order.id)}" ${canReorderOrder(order.id,true)?"":"disabled"}>${label}</button>`;
    return onNext ? `<span class="atlas-next-membership">On next load</span><details class="atlas-next-options"><summary>Priority options</summary>${action(pinned?"Remove priority":"Keep on next load")}</details>` : action(pinned?"Remove priority":"Next load");
  }

  function nextPriorityProposal(id, beforeId = null, fromDrop = false) {
    if (!canReorderOrder(id) || !storage()?.enabled || !savedDay.ready || savedDay.day !== find("[data-route-date]").value) throw Error("Open an editable saved day before prioritizing a load.");
    const source = state.analyzed.find(o=>o.id===id), next = state.loads[state.lockedTrips.length];
    if (!source || source.issues.length || !Number.isSafeInteger(source.palletSpaces) || source.palletSpaces<1) throw Error("Review this order’s SKU and box counts before prioritizing it.");
    if (assignmentVan(assignmentAt(state.lockedTrips.length))) throw Error("The next load uses a van. Review its vehicle and fit in Remaining Plan before changing its load.");
    if (source.palletSpaces>state.truckPalletTarget || next?.shipments.some(s=>!canReorderOrder(s.orderId))) throw Error("This load includes a split or partly sent order. Review its shipment allocations in Remaining Plan.");
    const currentIds = [...new Set(next?.shipments.map(s=>s.orderId) || [])];
    const remove = !fromDrop && state.nextLoadPriority?.includes(id);
    const ids = currentIds.filter(x=>x!==id);
    const position = beforeId ? ids.indexOf(beforeId) : -1; ids.splice(position < 0 ? ids.length : position,0,id);
    const spaces = ids.reduce((sum,x)=>sum+(state.analyzed.find(o=>o.id===x)?.palletSpaces || 0),0);
    const prior=[...(state.nextLoadPriority||[]),...state.analyzed.map(o=>o.id).filter(x=>!state.nextLoadPriority?.includes(x))];
    const after=remove?prior.filter(x=>x!==id).concat(id):[...ids,...prior.filter(x=>!ids.includes(x))];
    return {id,remove,ids,currentIds,spaces,overflow:spaces>state.truckPalletTarget,sequenceChanged:prior.join()!==after.join()};
  }

  function openNextPriority(id, beforeId = null, fromDrop = false) {
    let proposal,error; try {proposal=nextPriorityProposal(id,beforeId,fromDrop);} catch(e){error=e.message;}
    dispatchUI.move={priority:true,id,beforeId,fromDrop,owner:state.ownerId,day:savedDay.day,revision:savedDay.revision,document:JSON.stringify(dayDocument())};
    const o=dayOrders().find(o=>o.id===id), p=proposal;
    find("[data-dispatch-move]").innerHTML=`<h2>${p?.remove?"Remove next-load priority":p?.overflow?"Trip "+(state.lockedTrips.length+1)+" is at planning target":"Send on next load?"}</h2><p>${escape(o?.customer)} · ${escape(o?.orderNumber)}</p>${error?`<p role="alert">${escape(error)}</p>`:p.remove?"<p>This order returns to normal planning. Its SKU and box counts stay the same.</p>":`<p>${p.spaces} / ${state.truckPalletTarget} target pallets. ${p.overflow?"This order does not fit with the current next load.":"Prioritize this order for the next trip."}</p>${find("[data-route-preserve]").checked && p.sequenceChanged?"<p>This changes your saved stop order.</p>":""}`}<div class="atlas-route-card-tools"><button class="atlas-route-button" data-dispatch-close="move">Cancel</button>${error?"":p.overflow&&!p.remove?`<button class="atlas-route-button atlas-route-primary" data-next-confirm="later">Place on Trip ${state.lockedTrips.length+2}</button><button class="atlas-route-button" data-next-capacity>Review load capacity</button>`:`<button class="atlas-route-button atlas-route-primary" data-next-confirm="apply">${p.remove?"Remove priority":"Use next load"}</button>`}</div>`;
    find("[data-dispatch-move]").showModal();
  }

  async function applyNextPriority(action) {
    const m=dispatchUI.move;
    if(!m?.priority || m.owner!==state.ownerId || m.day!==savedDay.day || m.revision!==savedDay.revision || m.document!==JSON.stringify(dayDocument())){find("[data-dispatch-move]").close();return;}
    try {
      const p=nextPriorityProposal(m.id,m.beforeId,m.fromDrop);
      if(p.remove) state.nextLoadPriority=state.nextLoadPriority.filter(id=>id!==m.id);
      else if(p.overflow){
        if(action!=="later")return;
        state.nextLoadPriority=p.currentIds;
        // With current next load pinned, this order is first in the ordinary queue.
        const order=state.orders.find(o=>o.id===m.id);
        state.orders=[order,...state.orders.filter(o=>o.id!==m.id)];
      } else state.nextLoadPriority=p.ids;
      find("[data-dispatch-move]").close();renderOrders();flashOrderPlacement(m.id);
      await saveDay();
      if(m.owner!==state.ownerId || m.day!==savedDay.day)return;
      if(savedDay.dirty){savedDay.message="Priority not saved. Select Save Day to retry.";showSaveStatus();return;}
      if(window.atlasRoutingConnection?.available && !state.lockedTrips.length) await optimizeDay();
  ...
  function openDispatchMove(id, trip = state.lockedTrips.length) {
    ...
    if(state.nextLoadPriority?.length){find("[data-route-progress]").textContent="Remove next-load priorities before using Move to change other trips.";return;}
  ...
  function flashOrderPlacement(id) {
    document.getElementById("atlasDeliveryRouting").querySelectorAll("[data-route-row]").forEach(row=>{
      ...
      const check=document.createElement("span");check.className="atlas-route-placement-check";check.textContent="✓ Moved";check.setAttribute("role","status");row.querySelector(".atlas-route-order-link,.atlas-route-mobile-order-info strong")?.after(check);setTimeout(()=>check.remove(),2200);
    });
  }

  function dayDocument() {
    const upgraded = driverDirectory ? Object.fromEntries(Object.entries(state.assignments).map(([key,value])=>[key,assignmentAPI().validate(assignmentAPI().resolve(value,driverDirectory.candidates))])) : state.assignments;
    return window.atlasRoutingStorage.document({ schemaVersion: driverDirectory || Object.values(state.assignments).some(a=>typeof a==='object') ? 4 : 3, date: find("[data-route-date]").value, orders: dayOrders(), catalog: state.catalog,
      ...
      assignments: upgraded, vanConfirmed: state.vanConfirmed, lockedTrips: state.lockedTrips.map((trip,index)=>({...trip,assignment:upgraded[index] || trip.assignment})), ...(state.nextLoadPriority === null ? {} : {nextLoadPriority: state.nextLoadPriority}) });
  }
  ...
  function showSaveStatus() {
    ...
    const canEditOrder = intakeSnapshot().canEdit;
    document.getElementById("atlasDeliveryRouting").querySelectorAll("[data-route-edit],[data-route-delete]").forEach(button => { button.disabled = !canEditOrder; });
  ...
  function applyReadOnlyControls() {
    ...
    const controls = '[data-route-optimize],[data-route-intake],[data-route-edit],[data-route-drag],[data-route-move],[data-route-assignment],[data-route-driver],[data-route-vehicle],[data-route-schedule],[data-route-confirm-van],[data-route-lock-next],[data-route-reopen-last],[data-route-save-day],[data-route-confirm-day],[data-dispatch-promote],[data-dispatch-choose],[data-dispatch-apply],[data-next-priority],[data-next-confirm],[data-dispatch-settings] input,[data-dispatch-settings] select';
  ...
  function showReminderStatus() {
    ...
    const label=status.enabled?"Enabled on this device":window.Notification?.permission==="denied"?"Notifications blocked":!reminders()?"Setup unavailable":"Not enabled";
    find("[data-route-reminder-status]").textContent = `${label} · ${status.message}`;
  ...
  function applySavedDay(result, { preserveDraft = false } = {}) {
    ...
    state.assignments = doc?.assignments || {}; state.vanConfirmed = doc?.vanConfirmed || {}; state.lockedTrips = doc?.lockedTrips || []; state.nextLoadPriority = doc?.nextLoadPriority ?? null;
  ...
  async function loadDay(review = false) {
    ...
    find(".atlas-route-main").inert = true;
    showSaveStatus();
    try {
  ...
  async function searchHistory(offset = 0, paging = false) {
    ...
    } catch (error) { if (current()) status.textContent = error.code === "DISABLED" ? "Previous Trips search is awaiting the private database update. Existing saved days can still be opened by date in the connected preview." : error.message; }
  }
  ...
  function updateOptimizeAvailability() {
    const button = find("[data-route-optimize]"); if (!button) return;
    const reason = accessReadOnly || driverMode ? "Only administrators can optimize routes." :
      savedDay.busy || intakeRequest || state.planningController ? "Wait for the current operation to finish." :
      !window.atlasRoutingConnection?.available ? "Connect routing to calculate traffic and timing." :
      !dayOrders().length ? "Scan delivery orders on mobile before optimizing." :
      !state.loadPlan?.trips.length || state.loadPlan?.unscheduled.length ? "Review incomplete orders and loads before optimizing." : "";
    button.disabled = !!reason; button.title = reason || "Calculate routes with current traffic";
  }

  function orderActions(order) {
    const editable = intakeSnapshot().canEdit;
    const locked = isLockedOrder(order.id) || order.dispatchedOn || order.deliveredOn;
    return `<div class="atlas-route-order-actions">${priorityAction(order)}<button type="button" class="atlas-route-button" data-route-edit="${escape(order.id)}" ${editable ? "" : "disabled"}>${icon("pencil")}Edit</button><button type="button" class="atlas-route-button atlas-route-danger" data-route-delete="${escape(order.id)}" ${editable ? "" : "disabled"} title="${locked ? "Sent-out or completed order: reopen its latest trip before deletion is available." : "Remove this unsent order"}">${icon("trash")}Delete</button></div>`;
  }

  function deleteBlockReason(id) {
    if (!intakeSnapshot().canEdit || !storage()?.enabled || !savedDay.ready || savedDay.day !== find("[data-route-date]").value) return "Open an editable saved day before deleting an order.";
    const order = dayOrders().find(o => o.id === id);
    if (!order) return "This order is no longer in the selected day.";
    if (isLockedOrder(id) || order.dispatchedOn || order.deliveredOn) return "This order belongs to a sent-out or completed load. Reopen its latest trip first when permitted.";
    return "";
  }

  function requestDeleteOrder(id) {
    const reason = deleteBlockReason(id), order = dayOrders().find(o => o.id === id);
    if (documentFlow?.active()) documentFlow.reset();
    const dialog = find("[data-route-delete-dialog]");
    deleteRequest = reason ? null : { id, owner: state.ownerId, day: savedDay.day, revision: savedDay.revision, generation: savedDay.generation, document: JSON.stringify(dayDocument()) };
    const analyzed = state.analyzed?.find(o => o.id === id);
    dialog.innerHTML = `<small class="atlas-route-eyebrow">DELETE DELIVERY ORDER</small><h2 id="atlasRouteDeleteTitle">${reason ? "Order cannot be deleted" : `Remove ${escape(order.customer)} from today's deliveries?`}</h2><p>${reason ? escape(reason) : `${escape(order.orderNumber)} · ${analyzed?.palletSpaces ?? "Unknown"} pallets · ${window.atlasRoutingIntake.orderBoxCount(order.lines) ?? "Unknown"} boxes`}</p>${reason ? "" : '<p>This removes the order from unsent loads and recalculates the day.</p>'}<p role="alert" data-route-delete-error></p><div class="atlas-route-card-tools"><button type="button" class="atlas-route-button" data-route-delete-cancel autofocus>Keep Order</button>${reason ? "" : '<button type="button" class="atlas-route-button atlas-route-danger" data-route-delete-confirm>Delete Order</button>'}</div>`;
    dialog.showModal();
  }

  async function confirmDeleteOrder() {
    const request = deleteRequest; if (!request || deletingOrder) return;
    const reason = deleteBlockReason(request.id);
    if (reason || request.owner !== state.ownerId || request.day !== savedDay.day || request.revision !== savedDay.revision || request.generation !== savedDay.generation || request.document !== JSON.stringify(dayDocument())) {
      find("[data-route-delete-error]").textContent = reason || "The day changed. Close this dialog and review the order again."; return;
    }
    const before = { orders: state.orders, nextLoadPriority: state.nextLoadPriority, assignments: { ...state.assignments }, vanConfirmed: { ...state.vanConfirmed }, planned: state.planned, vehicleChanges: { ...vehicleChanges }, saved: { ...savedDay } };
    const current = () => state.open && state.ownerId === request.owner && window.AtlasAuth?.getSession()?.user?.id === request.owner && savedDay.generation === request.generation && savedDay.day === request.day;
    deletingOrder = true; savedDay.busy = true;
    find("[data-route-delete-dialog]").querySelectorAll("button").forEach(b => b.disabled = true);
    find(".atlas-route-main").inert = true;
    try {
      state.orders = state.orders.filter(o => o.id !== request.id);
      if (state.nextLoadPriority) state.nextLoadPriority = state.nextLoadPriority.filter(id => id !== request.id);
      renderOrders();
      ...
      if (!current()) return;
      state.orders = before.orders; state.nextLoadPriority = before.nextLoadPriority; state.assignments = before.assignments; state.vanConfirmed = before.vanConfirmed;
      Object.assign(savedDay, before.saved); vehicleChanges = before.vehicleChanges;
      renderOrders({ keepAssignments: true, dirty: false });
      if (before.planned) renderTimedPlan(before.planned);
      find("[data-route-delete-error]").textContent = "The order could not be deleted. Nothing was changed.";
    } finally {
      if (current()) {
        deletingOrder = false; savedDay.busy = false; find(".atlas-route-main").inert = false;
        find("[data-route-delete-dialog]").querySelectorAll("button").forEach(b => b.disabled = false);
        showSaveStatus(); renderOrders({ keepAssignments: true, dirty: false });
        if (before.planned && state.orders === before.orders) renderTimedPlan(before.planned);
      }
    }
  }

  async function publishDriverTrip(index) {
    if(accessReadOnly || !savedDay.canEdit || savedDay.busy || !state.loads[index])return;
    try {
      if(!state.assignments[index]){state.assignments[index]=assignmentAt(index);savedDay.dirty=true;}
      if(savedDay.dirty)await saveDay();
      if(savedDay.dirty || savedDay.busy)throw Error("Save the day before publishing its driver assignment.");
      const payload={p_day:savedDay.day,p_revision:savedDay.revision,p_trip:index};
      await assignmentAPI().request('publish_ready',{...payload,p_plan:state.loads.map(load=>load.shipments.map(sh=>({orderId:sh.orderId,palletSpaces:sh.palletSpaces,boxAllocation:sh.boxAllocation}))),p_shipments:state.loads[index].shipments.map(sh=>({orderId:sh.orderId,palletSpaces:sh.palletSpaces,boxAllocation:sh.boxAllocation}))});
      savedDay.message="Assigned trip published. The employee can open it from Home.";
    } catch(error){savedDay.message=error.message==='SPLIT_ALLOCATION_REQUIRES_SENT_PLAN'?"This split order needs its full sent shipment allocation before publishing driver access.":error.message;}
    showSaveStatus();
  }
  ...
  async function lockNextTrip() {
    ...
    if (state.nextLoadPriority) state.nextLoadPriority = state.nextLoadPriority.filter(id => !snapshot.shipments.some(s => s.orderId === id));
    for (const order of dayOrders()) {
  ...
  async function importCatalog(event) {
    ...
    const draft=event.target.closest("[data-settings-form]");
    ...
    try {
      ...
      if(draft){if(draft.isConnected&&draft.closest("dialog").open){draft.catalogDraft=catalog;status.textContent=`${catalog.length} product specifications · Save settings to apply`;}return;}
      state.catalog = catalog;
      status.textContent = `${catalog.length} product specifications`;
      renderOrders();
    } catch (error) {
  ...
  function openDraft(id = null, show = true) {
    if (intakeRequest || savedDay.busy || state.planningController || accessReadOnly || driverMode || (storage()?.enabled && (!savedDay.ready || !savedDay.canEdit))) return;
    draftGeneration++;
    cancelPhotoReading();
  ...
  async function refreshScannerStatus(startup = false) {
    const owner=state.ownerId, scanDay=find("[data-route-date]").value;
    const blocked=()=>startup ? savedDay.busy || !savedDay.ready || !!state.planningController || !!intakeRequest || documentFlow?.active() : liveBusy();
    if(!state.open||driverMode||blocked()||document.hidden||!storage()?.scannerStatus)return;
    try {
    ...
    } catch (error) { scannerReady=false; if(startup)throw error; }
  }
  ...
  function initializeDocumentFlow(section) {
    documentFlow = window.atlasRoutingIntake.createDocumentFlow({ host: section, icon, snapshot: intakeSnapshot,
      ...
      upload: async order => {
        ...
        const seed={...dayDocument(),date:order.date,orders:[],lockedTrips:[],assignments:{},vanConfirmed:{}}; delete seed.nextLoadPriority;
      ...
      remove: requestDeleteOrder,
      map: () => openRoutingDrawer("map"),
    ...
    more.innerHTML = '<h2>More day actions</h2><p data-route-mobile-save-detail></p>' + [["save","Save changes / retry"],["queue","Waiting loads"],["history","Previous Trips"],["load","View Saved Day"],["reminders","Delivery Reminders"],["confirm","All Delivered"],["map","Map"],["settings","Settings"],["pods","Delivery documents"],["menu","Main Menu"]].map(([id,label]) => `<button type="button" class="atlas-route-button" data-manager-more="${id}">${label}</button>`).join("") + '<button type="button" class="atlas-route-button" data-manager-more="close">Close</button>'; section.append(more);
    nav.addEventListener("click", event => {
      ...
      else if (tab === "more") { renderMobileDashboard(); more.showModal(); }
    ...
    more.addEventListener("click", event => {
      ...
      if (["map", "settings"].includes(action)) openRoutingDrawer(action);
      if (mobileDayActions[action]) find(mobileDayActions[action])?.click();
      if (action === "menu") document.querySelector(".premium-menu-button")?.click();
    });
  }

  function openCapture() { if (window.matchMedia("(max-width:750px)").matches) return startScannerIntent(); if (intakeSnapshot().canEdit) documentFlow.open(); }
    ...
    if (date !== selectedDay) {
      ...
      try {
        if (storage()?.enabled) {
          ...
          const fallback = { ...dayDocument(), date, orders: [], assignments: {}, vanConfirmed: {}, lockedTrips: [] }; delete fallback.nextLoadPriority;
          savedDay.busy = true; showSaveStatus();
  ...
  function installOrderDragging(section) {
    ...
    const activate = (drag) => {
      ...
      if (drag.row.classList.contains("atlas-route-mobile-order")) { layer.classList.add("atlas-route-mobile-drag"); layer.appendChild(clone); }
      else { body.appendChild(clone); table.appendChild(body); layer.appendChild(table); }
    ...
    section.addEventListener("click", (event) => {
      if (Date.now() < suppressClickUntil && event.target.closest("[data-route-edit],[data-route-mobile-reorder]")) { suppressClickUntil = 0; event.preventDefault(); event.stopImmediatePropagation(); }
    }, true);
    section.addEventListener("pointerdown", (event) => {
      const row = event.target.closest("[data-route-row]"), handle = event.target.closest("[data-route-drag],[data-route-mobile-reorder]");
    ...
    section.addEventListener("pointermove", (event) => {
      ...
      drag.next = !!trip?.matches("[data-next-load-primary],[data-dispatch-next]") && matchMedia("(min-width:751px)").matches;
      const drop = find("[data-route-next-drop]"); if(drop){drop.classList.toggle("is-drop-target",drag.next); const o=state.analyzed.find(o=>o.id===drag.id), next=state.loads[state.lockedTrips.length]; const proposed=(next?.palletSpaces||0)+(next?.shipments.some(s=>s.orderId===drag.id)?0:o?.palletSpaces||0); drop.querySelector("[data-route-drop-preview]").textContent=drag.next?`${proposed} / ${state.truckPalletTarget} target pallets · release to review`:"Or use the order’s load actions.";}
    ...
    section.addEventListener("pointerup", (event) => {
      const drag = state.drag;
      if (!drag || drag.pointerId !== event.pointerId) return;
      const { id, target, active, trip, next } = drag;
      cancelOrderDrag();
      const sourceTrip = state.loads.findIndex(load=>load.shipments.some(s=>s.orderId===id));
      if (active && next) openNextPriority(id,target,true);
      else if (active && trip != null && trip !== sourceTrip) openDispatchMove(id, trip);
  ...
  function moveOrder(id, targetId) {
    ...
    if(state.nextLoadPriority?.length){find("[data-route-progress]").textContent="Remove next-load priorities before changing the overall stop order.";return;}
    const day = dayOrders();
  ...
  function renderOrders({ keepAssignments = false, dirty = true } = {}) {
    ...
    invalidatePlan();
    if (!keepAssignments) { state.assignments = {...state.assignments,...Object.fromEntries(state.lockedTrips.map((trip, index) => [index, trip.assignment]))}; state.vanConfirmed = Object.fromEntries(state.lockedTrips.map((trip, index) => [index, trip.vanConfirmed])); vehicleChanges = {}; }
    ...
    find("[data-route-orders]").innerHTML = analyzed.length ? analyzed.map((result, index) => {
      ...
      return `<tr data-route-row="${order.id}"><td class="atlas-route-order-position"><button type="button" data-route-drag="${order.id}" aria-label="Drag ${escape(order.customer)} to reorder" title="Drag to reorder">⠿ ${index + 1}</button><div><button type="button" aria-label="Move ${escape(order.customer)} up" data-route-move="-1" data-id="${order.id}" ${index === 0 ? "disabled" : ""}>↑</button><button type="button" aria-label="Move ${escape(order.customer)} down" data-route-move="1" data-id="${order.id}" ${index === orders.length - 1 ? "disabled" : ""}>↓</button></div></td><td><button type="button" class="atlas-route-order-link" data-route-edit="${order.id}">${escape(order.customer)}</button><small>${escape(order.orderNumber)}${state.nextLoadPriority?.includes(order.id) ? " · Prioritized" : ""}${order.photos.length ? ` · ${order.photos.length} photos` : ""}</small><small>${escape(isLockedOrder(order.id) && !order.dispatchedOn ? "Partially sent out" : window.atlasRoutingStorage.deliveryStatus(order).label)}${window.atlasRoutingStorage.deliveryStatus(order).date ? ` · ${escape(window.atlasRoutingStorage.deliveryStatus(order).date)}` : ""}</small>${result.issues.length ? '<small class="atlas-route-warning">Review needed</small>' : ""}</td><td>${escape(order.city)}</td><td><strong>${result.palletSpaces ?? "—"}</strong></td><td>${escape(order.timeWindow)}</td><td>${order.checkOnDelivery ? '<strong class="atlas-route-check-badge">CHECK ON DELIVERY</strong>' : ""}${escape(order.checkOnDelivery && /^CHECK\s+ON\s+DELIVERY$/i.test(order.notes) ? "" : order.notes)}${!result.issues.length && !order.dispatchedOn && !order.deliveredOn && !isLockedOrder(order.id) ? '<span class="atlas-route-ready">Ready</span>' : ""}${result.issues.length ? `<details><summary>Review ${result.issues.length} item${result.issues.length === 1 ? "" : "s"}</summary>${result.issues.map((issue) => `<p>${escape(issue)}</p>`).join("")}</details>` : ""}</td><td>${orderActions(order)}</td></tr>`;
    }).join("") : '<tr><td colspan="7" class="atlas-route-empty">No deliveries for this day. Scan delivery orders on mobile to begin.</td></tr>';
    const plan = core.countTruckTrips(analyzed.map((order) => ({ ...order, palletSpaces: order.issues.some((issue) => issue.includes("item quantity") || issue.includes("case quantity")) ? null : order.palletSpaces })), { truckPalletTarget: state.truckPalletTarget, dailyTripTarget: state.dailyTripTarget, lockedTrips: state.lockedTrips, nextLoadPriority: state.nextLoadPriority || [] });
    state.loads = plan.trips;
    state.loadPlan = plan;
    state.analyzed = analyzed;
    find("[data-route-orders]")?.querySelectorAll("tr[data-route-row]").forEach(row=>{const order=orders.find(o=>o.id===row.dataset.routeRow);if(order)row.lastElementChild.innerHTML=orderActions(order);});
    plan.trips.forEach((trip, index) => {
      if (trip.locked) return;
      const resolved = core.resolveVanAssignment(assignmentAt(index), trip.shipments, state.catalog);
      if (!resolved.switched) return;
      state.assignments[index] = resolved.assignment;
      state.vanConfirmed[index] = false;
      vehicleChanges[index] = `Trip ${index + 1} moved to Box Truck: ${resolved.fit.reason}${resolved.fit.percent == null ? "" : ` (${resolved.fit.percent}%)`}.`;
    ...
    find("[data-route-plan]").innerHTML = !orders.length ? `<div class="atlas-route-plan-empty"><span class="atlas-route-empty-icon">${icon("route")}</span><strong>Your delivery plan starts here</strong><p>Scan delivery orders on mobile and load product specifications to see trip cards and estimates.</p></div>` : `
      ...
      ${plan.trips.map((trip, index) => `<article class="atlas-route-trip"><span class="atlas-route-trip-number">${index + 1}</span><div><header><strong>Trip ${index + 1}</strong>${driverDirectory || typeof assignmentAt(index) === "object" ? assignmentControls(index,trip.locked) : `<label class="atlas-route-assignment">Driver / vehicle<select data-route-assignment="${index}" aria-label="Driver and vehicle for trip ${index + 1}" ${trip.locked ? "disabled" : ""}>${[["Bubba:truck", "Bubba · Box Truck"], ["Bubba:van1", "Bubba · Van 1"], ["Bubba:van2", "Bubba · Van 2"], ["Achmad:van1", "Achmad · Van 1"], ["Achmad:van2", "Achmad · Van 2"]].map(([value, label]) => `<option value="${value}" ${(assignmentAt(index)) === value ? "selected" : ""}>${label}</option>`).join("")}</select></label>`}<span class="atlas-route-trip-stats"><b>${assignmentVan(assignmentAt(index)) ? `${trip.palletSpaces} pallet equivalents` : `${trip.palletSpaces} / ${state.truckPalletTarget} target pallets`}</b><span>${trip.shipments.length} stop${trip.shipments.length === 1 ? "" : "s"}</span><span>Timing pending</span></span></header>${trip.locked ? `<small class="atlas-route-check-badge">Sent out ${escape(trip.sentOn)} - load locked</small>` : ""}${trip.needsWarehouseReview ? '<small class="atlas-route-check-badge">Warehouse fit review · above usual 11 pallets</small>' : ""}${trip.needsScheduleReview ? '<small class="atlas-route-check-badge">Additional trip · schedule review</small>' : ""}<p class="atlas-route-trip-path">${icon("pin")}<span>Warehouse → ${trip.shipments.map((shipment) => `${escape(shipment.customer)} (${shipment.palletSpaces})`).join(" → ")} → Warehouse</span></p>${vehicleChanges[index] ? `<p class="atlas-route-planning-review" role="status" data-route-vehicle-change="${index}">${escape(vehicleChanges[index])}</p>` : ""}${vanLoadReview(trip, index)}${loadBoxList(trip)}${trip.shipments.some((shipment) => orders.find((order) => order.id === shipment.orderId)?.checkOnDelivery) ? '<small class="atlas-route-check-badge">CHECK ON DELIVERY on this trip</small>' : ""}</div></article>`).join("")}
  ...
  function renderCapacity(plan, orders) {
    ...
    const truckTrips = plan.trips.map((trip, index) => ({ ...trip, number: index + 1 }))
      .filter((trip) => !trip.locked);
    const latest = truckTrips[0], target = plan.targets.truckPalletTarget;
    ...
    const title = !orders.length ? `Truck target: ${target} pallets` : latest
      ? `Next planned load · Trip ${latest.number}: ${latest.palletSpaces} / ${target} target pallets`
  ...
  function invalidatePlan() {
    ...
    state.planningController?.abort(); state.planningController = null; state.planned = null; dispatchUI.timing="pending";
  ...
  function openTripSheet(tripIndex) {
    ...
    find("[data-route-sheet-content]").innerHTML = `
      <header class="atlas-route-sheet-header"><img class="atlas-route-sheet-logo" src="./chubby-gorilla-header-v2.png" alt="Chubby Gorilla" width="348" height="82" /><span>Trip Sheet</span><h1>Trip ${sheet.number} · ${escape(sheet.driver)}</h1><p>${escape(sheet.date)} · ${escape(sheet.vehicle)} · ${sheet.stops.length} stop${sheet.stops.length===1?"":"s"} · ${sheet.palletSpaces} pallet${sheet.palletSpaces===1?"":"s"}</p></header>
  ...
  function vanLoadReview(trip, index) {
    if (!assignmentVan(assignmentAt(index))) return "";
  ...
  function getReliefOptions(plan) {
    if(state.loads.some((load,index)=>typeof assignmentAt(index)==="object"))return [];
  ...
  function renderTimedPlan(plan) {
    ...
    find("[data-route-utilization]").textContent = plan.complete ? Object.entries(plan.drivers).filter(([, driver]) => driver.trips).map(([name, driver]) => `${driver.displayName || name} ${driver.utilizationPercent}%`).join(" · ") || "—" : "Pending";
    ...
    find(".atlas-route-relief strong").textContent = plan.drivers.Achmad?.trips ? `Achmad · ${plan.drivers.Achmad.trips} planned van trip${plan.drivers.Achmad.trips === 1 ? "" : "s"}` : driverDirectory ? "Assign available relief in trip settings" : "Achmad · standby";
    ...
    find("[data-route-plan]").innerHTML = `<div class="atlas-route-plan-banner"><span class="atlas-route-plan-icon">${icon("truck")}</span><div><strong>${plan.complete ? "DAILY ROUTE ESTIMATE" : "CALCULATION IN PROGRESS"}</strong><p>${plan.trips.length} calculated trips · ${plan.unscheduled.length} unscheduled shipments</p></div><p class="atlas-route-plan-status">${issues.length || plan.unscheduled.length ? "Review needed" : plan.complete ? "Ready for review" : "Day is incomplete"}</p></div>
      ...
      ${Object.entries(plan.drivers).filter(([, driver]) => driver.trips).map(([name, driver]) => `<p class="atlas-route-intake-note">${escape(driver.displayName || name)}: ${driver.lunch ? `lunch ${displayTime(driver.lunch.start)}–${displayTime(driver.lunch.end)}` : "lunch reserved; timing pending"}${plan.complete ? `${driver.finishTime ? ` · Finish ${displayTime(driver.finishTime)}${driver.overtime ? " · After shift — review" : ""}` : ""} · ${Math.round(driver.workMinutes)} working minutes / 480 available` : ""}.</p>`).join("")}
  ...
  async function optimizeDay() {
    if (state.planningController || savedDay.busy || intakeRequest || accessReadOnly || driverMode || !window.atlasRoutingConnection?.available) return;
    const connection = window.atlasRoutingConnection;
    const loads = state.loads.map((load, index) => {
      const assigned = assignmentAPI().load(assignmentAt(index));
      return { ...load, ...assigned, warehouseConfirmed: Boolean(state.vanConfirmed[index]), palletTarget: state.truckPalletTarget };
    });
    ...
    const controller = new AbortController(); state.planningController = controller; dispatchUI.timing="updating"; renderDispatch();
    showSaveStatus();
    ...
    try {
      ...
      if (current()) { state.planningController = null; if(dispatchUI.timing!=="failed")dispatchUI.timing="ready";renderDispatch(); find("[data-route-stop]").hidden = true; updateOptimizeAvailability(); showSaveStatus(); }
    }
  }
  ...
  function leaveRouting(target) {
    ...
    if (target === "Dashboard") window.atlasOpenDashboard?.();
      ...
      if(target)window.AtlasNavigation?.open(({Home:"home",Inventory:"inventory","Browse Aisles":"aisles",Workflows:"coc"})[target] || target);
    }
    return true;
  }

  window.atlasLeaveRouting = () => !state.open || leaveRouting();
  ...
  function resetWorkspace() {
    ...
    find('[data-route-mobile-action="search"]')?.setAttribute("aria-expanded", "false");
    if (find("[data-route-mobile-query]")) find("[data-route-mobile-query]").value = "";
    ...
    entryGeneration++; driverMode = false; accessReadOnly = false; driverDirectory = null;
    cancelOrderDrag();
    window.atlasRoutingPOD?.reset();
    ...
    state.allUrls.clear();
    state.orders = [];
    state.truckPalletTarget = 11; state.dailyTripTarget = 3; state.assignments = {}; state.lockedTrips = []; state.nextLoadPriority = null; state.vanConfirmed = {};
  ...
  function paintScannerStartup(status, message = "Opening your scanner…") {
    const section = document.getElementById("atlasDeliveryRouting");
    section.hidden = false; section.inert = false; root.classList.add("atlas-routing-open");
    section.classList.add("is-opening-scanner"); section.dataset.scannerStartup = status;
    for (const child of section.children) if (!child.classList.contains("atlas-route-scanner-opening")) child.inert = true;
    find("[data-scanner-startup-status]").textContent = message;
    find("[data-scanner-recovery]").hidden = status === "opening";
    find(".atlas-scanner-spinner").hidden = status !== "opening";
  }

  function revealScannerDestination() {
    const section = document.getElementById("atlasDeliveryRouting");
    section.classList.remove("is-opening-scanner");
    for (const child of section.children) child.inert = false;
    section.dataset.scannerStartup = "ready";
  }

  function cancelScannerStartup() {
    clearTimeout(scannerStartup?.timer); scannerStartup = null; entryGeneration++;
    // Initialization has no new order to discard; stop late responses and camera work.
    if (!state.open) { resetWorkspace(); root.classList.remove("atlas-routing-open"); document.getElementById("atlasDeliveryRouting").hidden = true; }
    else { savedDay.busy = false; leaveRouting("home"); }
  }

  async function startScannerIntent(mode = "ready") {
    if (!window.AtlasAuth?.getSession()?.user?.id) { window.AtlasAuth?.open(); return; }
    mount(); clearTimeout(scannerStartup?.timer);
    const request = { mode, started: Date.now(), owner: window.AtlasAuth.getSession().user.id }; scannerStartup = request; scannerOwner = request.owner;
    paintScannerStartup("opening");
    request.timer = setTimeout(() => {
      if (scannerStartup !== request) return;
      paintScannerStartup("unavailable", "The scanner is still connecting. You can wait, retry, or return to ATLAS.");
    }, 8000);
    try {
      if (!navigator.onLine) throw Object.assign(Error("You’re offline. Reconnect to scan an order."),{scannerState:"offline"});
      await window.atlasRoutingConnectionReady?.({retry:mode==="retry"});
      if (scannerStartup !== request) return;
      await enterRouting();
      if (scannerStartup !== request) return;
      if (!savedDay.ready && storage()?.enabled) await loadDay();
      await refreshScannerStatus(true);
      if (scannerStartup !== request) return;
      if (driverMode || accessReadOnly || (savedDay.ready && !savedDay.canEdit)) throw Object.assign(Error("This account does not have permission to scan delivery orders."),{scannerState:"unauthorized"});
      if (!intakeSnapshot().canEdit || !scannerReady || !window.atlasRoutingConnection?.photoAvailable) throw Error("Order scanning could not connect. Try Again to reconnect, or return to ATLAS.");
      clearTimeout(request.timer); scannerStartup = null;
      // Modal is opened before revealing its background, in the same frame.
      revealScannerDestination();
      documentFlow.open("ADD_DOCUMENT", { direct: true, scanner: true });
    } catch (error) {
      if (scannerStartup !== request) return;
      clearTimeout(request.timer); scannerStartup = null;
      revealScannerDestination();
      paintScannerStartup(error.scannerState || (navigator.onLine ? "unavailable" : "offline"), error.message || "The scanner could not open. Try again.");
    }
  }

  async function enterRouting(assignedOnly = false) {
    if (state.open && (!assignedOnly || driverMode)) return;
    if (state.open && !leaveRouting()) return;
    const session = window.AtlasAuth?.getSession();
    if (!session?.user?.id) { window.AtlasAuth?.open(); return; }
    const entry = ++entryGeneration;
    await window.atlasRoutingConnectionReady?.();
    let access;
    ...
    mount();
    driverMode = assignedOnly || !!access && !["office", "viewer"].includes(access.capability);
    ...
    state.ownerId = session.user.id; state.open = true;
    renderOrders({ dirty: false });
    const section = document.getElementById("atlasDeliveryRouting");
    section.hidden = false; root.classList.add("atlas-routing-open"); window.scrollTo(0, 0);
    dispatchUI.tab = driverMode ? "pods" : "orders"; applyDispatchTab(); applyReadOnlyControls();
    if (driverMode) void window.atlasRoutingPOD?.load();
    else if (storage()?.enabled) {
      scannerReady = false; if(!accessReadOnly) await loadDriverDirectory(); await loadDay();
      if (entry === entryGeneration && state.open && state.ownerId === session.user.id) await refreshScannerStatus();
    }
  }

  window.atlasOpenRouting = options => options?.intent === "scan-order" ? startScannerIntent() : enterRouting(options?.intent === "my-trips").catch(error => window.alert(error.message));
  ...
  window.addEventListener("atlas-auth-changed", (event) => {
    if (scannerOwner && event.detail?.session?.user?.id !== scannerOwner) { cancelScannerStartup(); return; }
