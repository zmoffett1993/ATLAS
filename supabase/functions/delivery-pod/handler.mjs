...
export function filename(b){const match=/^(?:SO(?:[- ]US)?[- ])?([0-9]{1,20})$/.exec(String(b.sales_order).trim().toUpperCase().replace(/ +/g,' '));
...
export function createHandler({enabled=false,origins=[],authenticate,context,list,receive,putImmutable,download,email,emailEnabled=false}){
 return async request=>{
  ...
  try{
   ...
   if(type.includes('application/json')){
    ...
    if(input.action==='list'){if(!/^20\d\d-\d\d-\d\d$/.test(input.date||''))fail('INVALID_DAY');return json({...await list(token,input.date),email_enabled:emailEnabled});}
   ...
   if(receipt.state==='received')return json({id:receipt.id,state:'received',filename:name,email:emailEnabled?'pending':'disabled'});
   ...
   return json({id:final.id,state:'received',filename:name,email:emailEnabled?'pending':'disabled'});
