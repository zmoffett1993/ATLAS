import {assertStationAccess,authenticated,cors,encodeBase64,fail,hasRole,json,receiver,service,sha256,station,stationForWarehouse} from "../_shared/coc.ts";
...
const deliveryFields="id,status,report_snapshot,workbook_file_name,completed_at,sent_at,received_at,office_completed_at";
...
Deno.serve(async(req)=>{if(req.method==="OPTIONS")return new Response("ok",{headers:cors});try{
  ...
  if(action==="receiver-inbox"){
    ...
    const result=await db.from("coc_deliveries").select(deliveryFields).eq("station_id",device.station_id).in("status",["SENT","RECEIVED","OFFICE_COMPLETED"]).order("sent_at",{ascending:false}).limit(100);
  ...
  if(action==="mark-completed"){
    const checked=await db.from("coc_deliveries").select("id,status,report_snapshot").eq("id",body.deliveryId).eq("station_id",device.station_id).single();
    if(checked.error)throw checked.error;
    if(checked.data.status!=="OFFICE_COMPLETED"){
      const snapshot=checked.data.report_snapshot||{};
      if(!String(snapshot.invoiceNumber||"").trim()||!String(snapshot.ifNumber||"").trim())
        throw Object.assign(new Error("COC_FINAL_REFERENCES_REQUIRED"),{status:400});
      const pending=await db.from("coc_workbook_revisions").select("id").eq("delivery_id",body.deliveryId).eq("status","PENDING").limit(1);
      if(pending.error)throw pending.error;
      if(pending.data?.length)throw Object.assign(new Error("COC_PENDING_REVISION_REQUIRES_REVIEW"),{status:409});
    }
    const now=new Date().toISOString(),result=await db.from("coc_deliveries").update({status:"OFFICE_COMPLETED",office_completed_at:now,office_completed_by_user_id:user.id,updated_at:now}).eq("id",body.deliveryId).eq("station_id",device.station_id).eq("status","RECEIVED").select("id,status,office_completed_at").maybeSingle();if(result.error)throw result.error;if(result.data)await db.from("coc_delivery_events").insert({delivery_id:result.data.id,event_type:"COC_OFFICE_COMPLETED",actor_user_id:user.id,actor_device_id:device.id});if(result.data)return json(result.data);const existing=await db.from("coc_deliveries").select("id,status,office_completed_at").eq("id",body.deliveryId).eq("station_id",device.station_id).single();if(existing.error)throw existing.error;if(existing.data.status!=="OFFICE_COMPLETED")throw Object.assign(new Error("COC_MUST_BE_RECEIVED_FIRST"),{status:409});return json(existing.data)
  }
